...
function isImageAttachment(attachment: PayableUploadedDocument) {
...
function resolveAttachmentUrl(attachment: PayableUploadedDocument) {
...
export default function PayableDetailPage() {
  ...
  const [previewAttachment, setPreviewAttachment] = useState<PayableUploadedDocument | null>(null);
  ...
  const uploadedDocuments = Array.isArray(detail.uploadedDocuments)
    ? (detail.uploadedDocuments as PayableUploadedDocument[])
    : [];
          ...
          <SectionCard title="Uploaded Documents" className="border-slate-200 shadow-sm">
            {uploadedDocuments.length ? (
              ...
              <div className="rounded-2xl border border-dashed border-slate-200 p-6 text-center text-sm font-semibold text-slate-500">
                No uploaded documents for this payable record.
              </div>
            )}
          </SectionCard>
        </div>
