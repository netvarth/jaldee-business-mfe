...
export function useAuditLogSearchSchema() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const response = await api.get<SearchSchema>("/audit-logs/search/schema");
      setSchema(normalizeSearchSchema(response) ?? null);
    } catch (loadError) {
      setSchema(null);
      setError(
        loadError instanceof Error ? loadError.message : "Failed to load audit log search schema."
      );
    } finally {
      setLoading(false);
    }
