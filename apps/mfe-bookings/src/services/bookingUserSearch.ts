...
export interface BookingUserSearchSort {
  field: string;
  direction: string;
}

export interface BookingUserSearchRequestBody {
  view?: string;
  filters: {
    logic: "AND";
    conditions: Array<{
  ...
  sort: BookingUserSearchSort[];
  page: number;
  size: number;
}

export function buildBookingUserSearchBody({
  filterClauses,
  schema,
  page = 0,
  size = 100,
  ...
  return {
    ...
    filters: conditions.length > 0 ? { logic: "AND", conditions } : null,
    sort: buildBookingUserSearchSort(schema),
    page,
    size,
  };
}

export function formatAppliedBookingUserFilterSummary(
...
function buildBookingUserSearchSort(schema: SearchSchema | null | undefined): BookingUserSearchSort[] {
