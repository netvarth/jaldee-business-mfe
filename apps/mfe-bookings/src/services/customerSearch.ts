...
export interface CustomerSearchSort {
  field: string;
  direction: string;
}

export interface CustomerSearchRequestBody {
  view?: string;
  filters: {
    logic: "AND";
    conditions: Array<{
...
export function buildCustomerSearchBody({
  filterClauses,
  schema,
  page = 0,
  size = 100,
  ...
  return {
    ...
    filters:
      conditions.length > 0
        ...
        : null,
    sort: buildCustomerSearchSort(schema),
    page,
    size,
  };
}

export function formatAppliedCustomerFilterSummary(
...
function buildCustomerSearchSort(schema: SearchSchema | null | undefined): CustomerSearchSort[] {
