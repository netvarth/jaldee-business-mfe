...
const formatBookingTime = (booking: BookingSummaryItem): string => booking.time || booking.timeRange || "All day";
const formatBookingStatus = (booking: BookingSummaryItem): string => booking.status || "Confirmed";

export default function SummaryModal({ summary, onClose }: SummaryModalProps): JSX.Element | null {
  ...
  const totalBookings = summary.totalBookings ?? entries.reduce((sum, entry) => sum + (entry.count ?? 0), 0) ?? 0;
  ...
  return (
    <Dialog
      ...
      onClose={() => onClose?.()}
      hideHeader
      overlayClassName="summary-backdrop"
      contentClassName="summary-modal"
      bodyClassName="summary-modal-body"
    >
      <div className="summary-header">
        <div className="summary-date-block">
          <span className="summary-date-number">{displayDate || summary.dateLabel || "—"}</span>
        ...
        <div className="summary-header-actions">
          ...
          <Button type="button" variant="outline" size="md" className="summary-close" onClick={onClose} aria-label="Close summary">
            ×
          </Button>
        </div>
      </div>
          ...
          return (
            ...
            >
              <div className="summary-row-top">
                <span className="summary-row-pill" style={{ background: entry.color || "#6c32ff" }}>
              ...
              {isExpanded && bookings.length > 0 && (
                <div className="summary-bookings-list">
                  {bookings.map((booking) => (
                    <div key={booking.id ?? `${booking.patient}-${booking.time}-${booking.timeRange}`} className="summary-booking-row">
