...
export default function StaffPage() {
  ...
  const navigateWithinUsersModule = useMemo(
    () => (to: string) => {
      ...
      routerNavigate(`/staff${normalized}`);
    },
    [routerNavigate],
  );
  ...
  const routeState = useMemo(() => {
    ...
    const usersIndex = pathSegments.indexOf("staff");
  ...
  const sharedModuleProps = {
    ...
    basePath: `${mfeProps.basePath}/staff`,
    ...
    uiOverrides: {
      listTitle: "Staff Overview",
      listSubtitle: "Create And Manage Staff's",
    },
  };
  ...
  return (
    <QueryClientProvider client={queryClient}>
      <SharedModulesProvider value={sharedModuleProps}>
        <div className="mfe-wrapper h-full overflow-y-auto">
          <UsersModule />
        </div>
