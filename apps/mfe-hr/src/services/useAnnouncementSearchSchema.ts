...
export function useAnnouncementSearchSchema() {
  const api = useHrApi();
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await api.get<SearchSchema>("/announcements/search/schema");
      setSchema(normalizeSearchSchema(response));
    } catch (loadError) {
      setSchema(null);
      setError(loadError instanceof Error ? loadError.message : "Failed to load announcement filters.");
    } finally {
      setLoading(false);
    }
  }, [api]);
