...
export function useAssetSearchSchema() {
  const api = useHrApi();
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await api.get<SearchSchema>("/assets/search/schema");
      setSchema(normalizeSearchSchema(response));
    } catch (loadError) {
      setSchema(null);
      setError(loadError instanceof Error ? loadError.message : "Failed to load asset filters.");
    } finally {
      setLoading(false);
    }
  }, [api]);
