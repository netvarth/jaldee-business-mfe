...
async function run() {
  ...
  const suffix = ENTERPRISE_DATA_SUFFIX;
  const primaryEmployeeName = "Rahul Sharma";
  ...
  async function slowTime(testId, timeStr, labelName) {
    ...
    const visible = await inputEl.waitFor({ state: "visible", timeout: 5000 }).then(() => true).catch(() => false);
    if (visible) {
      ...
      if (await trigger.isVisible().catch(() => false)) {
        ...
        const hourBtnVisible = await hourBtn.waitFor({ state: "visible", timeout: 1200 }).then(() => true).catch(() => false);
        if (hourBtnVisible) {
          await hourBtn.click().catch(() => { });
          await page.waitForTimeout(150);

          const minBtn = page.locator(`[data-testid="${testId}-min-${min}"]`).first();
          const minBtnVisible = await minBtn.waitFor({ state: "visible", timeout: 1000 }).then(() => true).catch(() => false);
          if (minBtnVisible) {
          ...
          const periodBtnVisible = await periodBtn.waitFor({ state: "visible", timeout: 1000 }).then(() => true).catch(() => false);
          if (periodBtnVisible) {
  ...
  async function clickFirstPrefix(prefix, labelName) {
    ...
    const visible = await target.waitFor({ state: "visible", timeout: 2500 }).then(() => true).catch(() => false);
    if (!visible) {
  ...
  async function slowSaveModal(scope) {
    ...
    const saveBtnVisible = await saveBtn.waitFor({ state: "visible", timeout: 5000 }).then(() => true).catch(() => false);
    if (saveBtnVisible) {
  ...
  try {
    ...
    if (loginFinished === true) {
      ...
      await page.waitForTimeout(1000);
      await slowType('[data-testid="signup-login-id-input"]', ENTERPRISE_LOGIN_ID, "Signup Login ID");
      await slowType('[data-testid="signup-tenant-name-input"]', enterpriseBrandName, "Business Name");
      await slowType('[data-testid="signup-first-name-input"]', "Dhyan", "First Name");
      await slowType('[data-testid="signup-last-name-input"]', "Global", "Last Name");
      await slowType('[data-testid="signup-password-input"]', ENTERPRISE_PASSWORD, "Signup Password");
    ...
    if (onboardingVisible) {
      ...
      await slowType('[data-testid="onboarding-company-name-input"]', enterpriseBrandName, "Onboarding Company Name");
      ...
      await page.waitForTimeout(onboardingPause);
      const businessUpdateResponsePromise = page.waitForResponse(
        (response) =>
          response.request().method() === "PUT"
          && /\/base-service\/v1\/api\/tenant\/[^/?]+(?:\?|$)/.test(response.url()),
        { timeout: 30000 },
      ).catch(() => null);
      await slowClick('[data-testid="onboarding-continue-button"]', "Continue Business Details");
      const businessUpdateResponse = await businessUpdateResponsePromise;
      const onboardingStepTwo = page.locator('[data-testid="onboarding-page"][data-state="step-2"]');
      const businessStepAdvanced = await onboardingStepTwo
        .waitFor({ state: "visible", timeout: 30000 })
        .then(() => true)
        .catch(() => false);
      if (!businessStepAdvanced) {
        const onboardingError = await page.locator(".onboarding-error").textContent().catch(() => "");
        const responseDetail = businessUpdateResponse
          ? `HTTP ${businessUpdateResponse.status()}: ${await businessUpdateResponse.text().catch(() => businessUpdateResponse.statusText())}`
          : "no tenant update response was received";
        throw new Error(`Business onboarding did not continue: ${onboardingError?.trim() || "no validation message was shown"} (${responseDetail})`);
      }
      await page.waitForTimeout(onboardingPause);
      ...
      await slowType('[data-testid="onboarding-location-name-input"]', `Bangalore HQ ${suffix} ${Date.now().toString().slice(-6)}`, "Location Name");
      await slowType('[data-testid="onboarding-pincode-input"]', "560001", "Pin Code");
      await slowType('[data-testid="onboarding-full-address-textarea"]', "Bangalore, Karnataka", "Full Address");
      ...
      const step4 = page.locator('[data-testid="onboarding-page"][data-state="step-4"]').first();
      const errorEl = page.locator(".onboarding-error").first();
      const success = await Promise.race([
        step4.waitFor({ state: "visible", timeout: 30000 }).then(() => true),
        errorEl.waitFor({ state: "visible", timeout: 30000 }).then(() => false)
      ]);
      if (!success) {
        const errText = await errorEl.textContent();
        throw new Error(`Location creation failed: ${errText?.trim() || "Unknown error"}`);
      }
      await page.waitForTimeout(3000);
    ...
    const deptList = [
      ...
      { name: `Product ${suffix}`, code: `PRD-${suffix}` },
      { name: `QA ${suffix}`, code: `QA-${suffix}` },
      { name: `DevOps & Cloud ${suffix}`, code: `DEV-${suffix}` },
      { name: `Security ${suffix}`, code: `SEC-${suffix}` },
      { name: `HR ${suffix}`, code: `HR-${suffix}` },
      { name: `Finance ${suffix}`, code: `FIN-${suffix}` },
      { name: `Sales ${suffix}`, code: `SAL-${suffix}` },
      { name: `Marketing ${suffix}`, code: `MKT-${suffix}` },
      { name: `Customer Success ${suffix}`, code: `CS-${suffix}` },
      { name: `IT Support ${suffix}`, code: `IT-${suffix}` },
    ];
    for (let i = 0; i < deptList.length; i++) {
      const d = deptList[i];
      const existingDepartment = page.getByText(d.name, { exact: true }).first();
      const existingDepartmentUpdated = page.getByText(`${d.name} Updated`, { exact: true }).first();
      if (await existingDepartment.isVisible().catch(() => false) || await existingDepartmentUpdated.isVisible().catch(() => false)) {
        console.log(`   [Skip] Department already exists: "${d.name}"`);
        if (i === 0 && await existingDepartmentUpdated.isVisible().catch(() => false)) {
          deptList[0].name = `${d.name} Updated`;
        }
        continue;
      }
      ...
      if (!(await slowClick('[data-testid="hr-settings-departments-add"], button:has-text("Add Department")', `Add ${d.name}`))) {
        throw new Error("Add Department button was not available");
      }
    ...
    if (departmentToEdit.name.endsWith("Updated")) {
      console.log(`   [Skip] Department already updated: "${departmentToEdit.name}"`);
    } else {
      ...
      await slowSaveModal("hr-settings-departments");
      await page.getByText(updatedDepartmentName, { exact: true }).first().waitFor({ state: "visible", timeout: 10000 });
    ...
    const levelList = [
      ...
      { num: "6", label: "Individual Contributors" },
      { num: "7", label: "Interns & Trainees" },
    ];
    ...
    const roleList = [
      ...
      { name: `VP Engineering ${suffix}`, code: `VPE-${suffix}`, level: "2", desc: "Vice President of Engineering" },
      { name: `Principal Architect ${suffix}`, code: `PRA-${suffix}`, level: "3", desc: "Principal Tech and Cloud Architect" },
      { name: `Senior Software Engineer ${suffix}`, code: `SSE-${suffix}`, level: "4", desc: "Leads engineering delivery" },
      { name: `QA Engineer ${suffix}`, code: `QAE-${suffix}`, level: "5", desc: "Quality assurance and testing" },
      { name: `Cloud Architect ${suffix}`, code: `CLA-${suffix}`, level: "3", desc: "DevOps & cloud platform architecture" },
      { name: `Product Manager ${suffix}`, code: `PRM-${suffix}`, level: "4", desc: "Product roadmap and alignment" },
      { name: `HRBP ${suffix}`, code: `HRB-${suffix}`, level: "5", desc: "HR Business Partner" },
      { name: `Accountant ${suffix}`, code: `ACC-${suffix}`, level: "6", desc: "Finance & accounts manager" },
      { name: `Sales Executive ${suffix}`, code: `SAE-${suffix}`, level: "6", desc: "Sales and regional accounts" },
      { name: `Customer Support Executive ${suffix}`, code: `CSE-${suffix}`, level: "6", desc: "Client support & success" },
      { name: `Helpdesk Engineer ${suffix}`, code: `HDE-${suffix}`, level: "6", desc: "IT Support and hardware" },
      { name: `Intern ${suffix}`, code: `INT-${suffix}`, level: "7", desc: "Engineering Intern" },
    ];
    for (let i = 0; i < roleList.length; i++) {
      const r = roleList[i];
      const existingRole = page.getByText(r.name, { exact: true }).first();
      const existingRoleUpdated = page.getByText(`${r.name} Updated`, { exact: true }).first();
      if (await existingRole.isVisible().catch(() => false) || await existingRoleUpdated.isVisible().catch(() => false)) {
        console.log(`   [Skip] Designation already exists: "${r.name}"`);
        if (i === 0 && await existingRoleUpdated.isVisible().catch(() => false)) {
          roleList[0].name = `${r.name} Updated`;
        }
        continue;
      }
      await closeAnyOpenModal();
    ...
    if (roleToEdit.name.endsWith("Updated")) {
      console.log(`   [Skip] Role already updated: "${roleToEdit.name}"`);
    } else {
      ...
      await slowSaveModal("hr-settings-designations");
      await page.getByText(updatedRoleName, { exact: true }).first().waitFor({ state: "visible", timeout: 10000 });
    ...
    const shiftList = [
      ...
      { name: `Morning Shift ${suffix}`, start: "06:00 AM", end: "02:00 PM" },
      { name: `Evening Shift ${suffix}`, start: "02:00 PM", end: "10:00 PM" },
      { name: `Night Shift ${suffix}`, start: "10:00 PM", end: "06:00 AM" },
      { name: `Flexible Shift ${suffix}`, start: "10:00 AM", end: "06:00 PM" },
      { name: `US Shift ${suffix}`, start: "06:00 PM", end: "03:00 AM" },
      { name: `Europe Shift ${suffix}`, start: "01:00 PM", end: "10:00 PM" },
    ];
    for (let i = 0; i < shiftList.length; i++) {
      const s = shiftList[i];
      const existingShift = page.getByText(s.name, { exact: true }).first();
      const existingShiftUpdated = page.getByText(`${s.name} Updated`, { exact: true }).first();
      if (await existingShift.isVisible().catch(() => false) || await existingShiftUpdated.isVisible().catch(() => false)) {
        console.log(`   [Skip] Shift already exists: "${s.name}"`);
        if (i === 0 && await existingShiftUpdated.isVisible().catch(() => false)) {
          shiftList[0].name = `${s.name} Updated`;
        }
        continue;
      }
      await closeAnyOpenModal();
    ...
    if (shiftToEdit.name.endsWith("Updated")) {
      console.log(`   [Skip] Shift already updated: "${shiftToEdit.name}"`);
    } else {
      ...
      await page.getByText(updatedShiftName, { exact: true }).first().waitFor({ state: "visible", timeout: 10000 });
      shiftToEdit.name = updatedShiftName;
      console.log(`   [Verified] Shift updated: "${updatedShiftName}"`);
    }

    console.log("\n>>> 5.5 SETTINGS - CREATE AND VERIFY SHIFT ROTATION...");
    await slowClick('[data-testid="hr-settings-shifts-tab-rotations"]', "Rotations Tab");
    await page.locator('[data-testid="hr-settings-rotations-panel"]').waitFor({ state: "visible", timeout: 10000 });
    const rotationName = `Enterprise Weekly Rotation ${suffix}`;
    const existingRotation = page.getByText(rotationName, { exact: true }).first();
    if (!(await existingRotation.isVisible().catch(() => false))) {
      await slowClick('[data-testid="hr-settings-rotations-add"]', "Add Shift Rotation");
      ...
      const shiftSelect = rotationModal.locator('[data-testid="hr-settings-rotations-add-shift"]');
      const shiftValues = await shiftSelect.locator("option").evaluateAll((options) => options
        .map((option) => option.value)
        .filter(Boolean));
      if (shiftValues.length < 2) throw new Error("Shift Rotation requires at least two available shifts for automation.");
      await shiftSelect.selectOption(shiftValues[0]);
      await shiftSelect.selectOption(shiftValues[1]);
      await rotationModal.locator(`[data-testid="hr-settings-rotations-sequence-${shiftValues[0]}"]`).waitFor({ state: "visible" });
      await rotationModal.locator(`[data-testid="hr-settings-rotations-sequence-${shiftValues[1]}"]`).waitFor({ state: "visible" });
      await rotationModal.locator(`[data-testid="hr-settings-rotations-sequence-up-${shiftValues[1]}"]`).click();
      await slowType('[data-testid="hr-settings-rotations-period"]', "7", "Days On Each Shift");
      await slowDate('[data-testid="hr-settings-rotations-startdate"]', futureDate(0), "Rotation Start Date");
      if (!(await rotationModal.locator('[data-testid="hr-settings-rotations-active"]').isChecked())) {
        await rotationModal.locator('[data-testid="hr-settings-rotations-active"]').check();
      }
    ...
    await page.getByText(rotationName, { exact: true }).first().waitFor({ state: "visible", timeout: 10000 });
    console.log(`   [Verified] Shift Rotation sequence created: "${rotationName}"`);
    ...
    const leaveList = [
      { name: `Casual Leave ${suffix}`, cat: "CASUAL", quota: "12" },
      { name: `Sick Leave ${suffix}`, cat: "SICK", quota: "12" },
      { name: `Earned Leave ${suffix}`, cat: "EARNED", quota: "18" },
      { name: `Maternity Leave ${suffix}`, cat: "MATERNITY", quota: "180" },
      { name: `Paternity Leave ${suffix}`, cat: "PATERNITY", quota: "15" },
      { name: `Bereavement Leave ${suffix}`, cat: "CASUAL", quota: "5" },
      { name: `Marriage Leave ${suffix}`, cat: "CASUAL", quota: "7" },
      { name: `Work From Home ${suffix}`, cat: "CASUAL", quota: "30" },
      { name: `Comp Off ${suffix}`, cat: "COMP_OFF", quota: "10" },
      { name: `Optional Holidays ${suffix}`, cat: "CASUAL", quota: "5" },
    ];
    for (let i = 0; i < leaveList.length; i++) {
      const l = leaveList[i];
      const existingLeave = page.getByText(l.name, { exact: true }).first();
      const existingLeaveUpdated = page.getByText(`${l.name} Updated`, { exact: true }).first();
      if (await existingLeave.isVisible().catch(() => false) || await existingLeaveUpdated.isVisible().catch(() => false)) {
        console.log(`   [Skip] Leave Type already exists: "${l.name}"`);
        if (i === 0 && await existingLeaveUpdated.isVisible().catch(() => false)) {
          leaveList[0].name = `${l.name} Updated`;
        }
        continue;
      }
      await closeAnyOpenModal();
    ...
    if (leaveTypeToEdit.name.endsWith("Updated")) {
      console.log(`   [Skip] Leave Type already updated: "${leaveTypeToEdit.name}"`);
    } else {
      ...
      await saveLeavePolicy();
      await page.getByText(updatedLeaveTypeName, { exact: true }).first().waitFor({ state: "visible", timeout: 10000 });
    ...
    const holidayList = [
      ...
      { name: `Holi ${suffix}`, date: "2026-03-03", type: "Public" },
      { name: `Good Friday ${suffix}`, date: "2026-04-03", type: "Public" },
      { name: `Vishu ${suffix}`, date: "2026-04-14", type: "Public" },
      { name: `Labour Day ${suffix}`, date: "2026-05-01", type: "Public" },
      { name: `Bakrid ${suffix}`, date: "2026-05-27", type: "Public" },
      { name: `Independence Day ${suffix}`, date: "2026-08-15", type: "Public" },
      { name: `Onam ${suffix}`, date: "2026-08-27", type: "Public" },
      { name: `Gandhi Jayanti ${suffix}`, date: "2026-10-02", type: "Public" },
      { name: `Diwali ${suffix}`, date: "2026-11-08", type: "Public" },
      { name: `Christmas ${suffix}`, date: "2026-12-25", type: "Public" },
    ];
    for (let i = 0; i < holidayList.length; i++) {
      const h = holidayList[i];
      const table = page.locator('[data-testid="hr-settings-holidays-table"]').first();
      const rows = await table.locator('tbody tr').allTextContents().catch(() => []);
      const dateExists = rows.some(r => r.includes(h.date));
      const nameExists = rows.some(r => r.includes(h.name) || r.includes(`${h.name} Updated`));
      if (dateExists || nameExists) {
        console.log(`   [Skip] Holiday already exists (by name or date in table): "${h.name}" / "${h.date}"`);
        if (i === 0 && rows.some(r => r.includes(`${h.name} Updated`))) {
          holidayList[0].name = `${h.name} Updated`;
        }
        continue;
      }
      await closeAnyOpenModal();
      ...
      const holidayClosed = await holidayModal.waitFor({ state: "hidden", timeout: 10000 }).then(() => true).catch(() => false);
      if (!holidayClosed) {
        const errorText = await holidayModal.locator('div[style*="e11d48"], [role="alert"], [data-testid$="-error"], .text-red-700').allTextContents().catch(() => []);
        throw new Error(`Holiday was not saved: ${errorText.join(" ").trim() || "modal remained open"}`);
      }
      await page.waitForTimeout(3000);
    }
    const holidayToEdit = holidayList[0];
    if (holidayToEdit.name.endsWith("Updated")) {
      console.log(`   [Skip] Holiday already updated: "${holidayToEdit.name}"`);
    } else {
      ...
      const editHolidayClosed = await editHolidayModal.waitFor({ state: "hidden", timeout: 10000 }).then(() => true).catch(() => false);
      if (!editHolidayClosed) {
        const errorText = await editHolidayModal.locator('div[style*="e11d48"], [role="alert"], [data-testid$="-error"], .text-red-700').allTextContents().catch(() => []);
        throw new Error(`Holiday was not saved: ${errorText.join(" ").trim() || "modal remained open"}`);
      }
      await page.getByText(updatedHolidayName, { exact: true }).first().waitFor({ state: "visible", timeout: 10000 });
    ...
    const officeOption = page.locator('[data-testid="hr-settings-attendance-allowedworkmodes-option-Office"]').first();
    await officeOption.waitFor({ state: "visible", timeout: 5000 });
    if (await officeOption.getAttribute("aria-selected") !== "true") {
      await slowClick('[data-testid="hr-settings-attendance-allowedworkmodes-option-Office"]', "Office Work Mode");
    }
    const homeOption = page.locator('[data-testid="hr-settings-attendance-allowedworkmodes-option-Home"]').first();
    await homeOption.waitFor({ state: "visible", timeout: 5000 });
    if (await homeOption.getAttribute("aria-selected") !== "true") {
    ...
    console.log("\n>>> SETTINGS - CREATE MULTI-OUTCOME ATTENDANCE POLICY RULE...");
    await openSettingsSection("policyrules", '[data-testid="hr-settings-policy-rules-add"]');
    await slowClick('[data-testid="hr-settings-policy-rules-tab-attendance"]', "Attendance Policy Tab");
    const policyRuleName = `Enterprise Late Arrival Rule ${suffix}`;
    const existingPolicyRule = page.getByText(policyRuleName, { exact: true }).first();
    if (!(await existingPolicyRule.isVisible().catch(() => false))) {
      await slowClick('[data-testid="hr-settings-policy-rules-add"]', "Add Attendance Policy Rule");
      const policyModal = page.locator('[data-testid="hr-settings-policy-rules-modal"]');
      await policyModal.waitFor({ state: "visible", timeout: 10000 });
      await slowType('[data-testid="hr-settings-policy-rules-name"]', policyRuleName, "Policy Rule Name");
      await slowSelect('[data-testid="hr-settings-policy-rules-condition"]', "LATE_MINUTES", "Late Minutes Condition");
      await slowSelect('[data-testid="hr-settings-policy-rules-operator-0"]', "BETWEEN", "First Outcome Operator");
      await slowType('[data-testid="hr-settings-policy-rules-value-0"]', "1", "First Outcome Start");
      await slowType('[data-testid="hr-settings-policy-rules-value-to-0"]', "15", "First Outcome End");
      await slowSelect('[data-testid="hr-settings-policy-rules-action-0"]', "REQUIRE_REGULARIZATION", "First Outcome Action");
      await slowClick('[data-testid="hr-settings-policy-rules-add-outcome"]', "Add Second Outcome");
      await policyModal.locator('[data-testid="hr-settings-policy-rules-outcome-1"]').waitFor({ state: "visible" });
      await slowSelect('[data-testid="hr-settings-policy-rules-operator-1"]', ">", "Second Outcome Operator");
      await slowType('[data-testid="hr-settings-policy-rules-value-1"]', "15", "Second Outcome Value");
      await slowSelect('[data-testid="hr-settings-policy-rules-action-1"]', "ALERT", "Second Outcome Action");
      await slowType('[data-testid="hr-settings-policy-rules-priority"]', "100", "Policy Priority");
      if (!(await policyModal.locator('[data-testid="hr-settings-policy-rules-active"]').isChecked())) {
        await policyModal.locator('[data-testid="hr-settings-policy-rules-active"]').check();
      }
      await policyModal.locator('[data-testid="hr-settings-policy-rules-save"]').click();
      await policyModal.waitFor({ state: "hidden", timeout: 20000 });
    }
    await page.getByText(policyRuleName, { exact: true }).first().waitFor({ state: "visible", timeout: 10000 });
    await slowClick('[data-testid="hr-settings-policy-rules-view-card"]', "Policy Card View");
    await page.locator('[data-testid^="hr-settings-policy-rule-card-"]').filter({ hasText: policyRuleName }).first().waitFor({ state: "visible", timeout: 10000 });
    await slowClick('[data-testid="hr-settings-policy-rules-view-table"]', "Policy Table View");
    console.log(`   [Verified] Multi-outcome Policy Rule created: "${policyRuleName}"`);
    ...
    const currentRunEmployeeName = primaryEmployeeName;
    ...
    async function createEmployees() {
      console.log(`\n>>> 8. EMPLOYEE MASTER - CREATE ${ENTERPRISE_EMPLOYEE_COUNT} ENTERPRISE EMPLOYEES...`);
      const firstNames = [
      ...
      const employeeProfiles = Array.from({ length: ENTERPRISE_EMPLOYEE_COUNT }, (_, i) => {
        ...
        const cycle = Math.floor(i / (firstNames.length * lastNames.length));
        return {
      ...
      for (let i = 0; i < employeeProfiles.length; i++) {
        ...
        if (searchFieldVisible) {
          ...
          const exists = await nameCell.waitFor({ state: "visible", timeout: 3000 }).then(() => true).catch(() => false);
          if (exists) {
        ...
        const panUniqueDigit = uniqueId.slice(-1);
        const panIndexDigits = String(i).padStart(3, "0");
        await slowType('input[placeholder*="ABCDE1234F"]', `ABCDE${panUniqueDigit}${panIndexDigits}F`, `IT Employee ${i + 1} PAN`);
        ...
        await slowType('[data-testid="hr-new-employee-base-salary"]', String(35000 + (i % 12) * 7500), `Base Salary ${i + 1}`);
    ...
    async function createRecruitmentRecords() {
      ...
      for (let i = 0; i < reqList.length; i++) {
        ...
        const createdHiringManager = primaryEmployeeName;
    ...
    async function runEmployeeViewActions() {
      ...
      const currentEmployeeName = primaryEmployeeName;
    ...
    async function runAttendanceAndLeaveActions() {
      ...
      if (!/Clocked in|Clocked out|already has an attendance|no shift was assigned/i.test(punchResult || "")) throw new Error(`Attendance punch failed: ${punchResult}`);
      ...
      if (await effectiveShift.isVisible()) {
        ...
        if (!effectiveShiftText || effectiveShiftText === "—" || effectiveShiftText === "-") {
          throw new Error("Attendance API did not return a resolved effective shift or a no-shift flag.");
        }
    ...
    async function runOrganizationChart() {
      ...
      await slowType('input[placeholder*="Search"]', primaryEmployeeName, "Organization Name Search");
    ...
    async function runOrganizationBranches() {
      ...
      await page.getByLabel("Employee").selectOption({ label: primaryEmployeeName });
      await page.getByRole("button", { name: "Save", exact: true }).click();
    ...
    async function runOrganizationTransfers() {
      ...
      await page.locator("tr").filter({ hasText: primaryEmployeeName }).first().waitFor({ state: "visible", timeout: 15000 });
    ...
    async function runPayrollActions() {
      ...
      const payrollComponentName = "Basic";
      const payrollStructureName = "Engineering Payroll";
      await visitHr("/hr/payroll/components", "PAYROLL COMPONENTS");
      const payrollComponents = [
        { name: "Basic", code: "BASIC", type: "EARNING", category: "BASIC", calculation: "PERCENTAGE", statutory: false, taxable: true, gross: true, net: true, ctc: true, payslip: true },
        { name: "HRA", code: "HRA", type: "EARNING", category: "HRA", calculation: "FORMULA", statutory: false, taxable: true, gross: true, net: true, ctc: true, payslip: true },
        { name: "Special Allowance", code: "SPECIAL_ALLOWANCE", type: "EARNING", category: "ALLOWANCE", calculation: "FORMULA", statutory: false, taxable: true, gross: true, net: true, ctc: true, payslip: true },
        { name: "Car Allowance", code: "CAR_ALLOWANCE", type: "EARNING", category: "ALLOWANCE", calculation: "FIXED_AMOUNT", statutory: false, taxable: true, gross: true, net: true, ctc: true, payslip: true },
        { name: "Fuel Allowance", code: "FUEL_ALLOWANCE", type: "EARNING", category: "ALLOWANCE", calculation: "FIXED_AMOUNT", statutory: false, taxable: true, gross: true, net: true, ctc: true, payslip: true },
        { name: "Driver Allowance", code: "DRIVER_ALLOWANCE", type: "EARNING", category: "ALLOWANCE", calculation: "FIXED_AMOUNT", statutory: false, taxable: true, gross: true, net: true, ctc: true, payslip: true },
        { name: "Conveyance", code: "CONVEYANCE", type: "EARNING", category: "ALLOWANCE", calculation: "FIXED_AMOUNT", statutory: false, taxable: true, gross: true, net: true, ctc: true, payslip: true },
        { name: "Medical Allowance", code: "MEDICAL_ALLOWANCE", type: "EARNING", category: "MEDICLAIM", calculation: "FIXED_AMOUNT", statutory: false, taxable: true, gross: true, net: true, ctc: true, payslip: true },
        { name: "Internet Allowance", code: "INTERNET_ALLOWANCE", type: "EARNING", category: "ALLOWANCE", calculation: "FIXED_AMOUNT", statutory: false, taxable: true, gross: true, net: true, ctc: true, payslip: true },
        { name: "Food Allowance", code: "FOOD_ALLOWANCE", type: "EARNING", category: "ALLOWANCE", calculation: "FIXED_AMOUNT", statutory: false, taxable: true, gross: true, net: true, ctc: true, payslip: true },
        { name: "Meal Allowance", code: "MEAL_ALLOWANCE", type: "EARNING", category: "ALLOWANCE", calculation: "FIXED_AMOUNT", statutory: false, taxable: true, gross: true, net: true, ctc: true, payslip: true },
        { name: "Mobile Allowance", code: "MOBILE_ALLOWANCE", type: "EARNING", category: "ALLOWANCE", calculation: "FIXED_AMOUNT", statutory: false, taxable: true, gross: true, net: true, ctc: true, payslip: true },
        { name: "Travel Allowance", code: "TRAVEL_ALLOWANCE", type: "EARNING", category: "ALLOWANCE", calculation: "FIXED_AMOUNT", statutory: false, taxable: true, gross: true, net: true, ctc: true, payslip: true },
        { name: "Shift Allowance", code: "SHIFT_ALLOWANCE", type: "EARNING", category: "ALLOWANCE", calculation: "FIXED_AMOUNT", statutory: false, taxable: true, gross: true, net: true, ctc: true, payslip: true },
        { name: "Night Shift Allowance", code: "NIGHT_SHIFT_ALLOWANCE", type: "EARNING", category: "ALLOWANCE", calculation: "FIXED_AMOUNT", statutory: false, taxable: true, gross: true, net: true, ctc: true, payslip: true },
        { name: "Overtime", code: "OVERTIME", type: "EARNING", category: "OVERTIME", calculation: "FIXED_AMOUNT", statutory: false, taxable: true, gross: true, net: true, ctc: true, payslip: true },
        { name: "Incentive", code: "INCENTIVE", type: "EARNING", category: "BONUS", calculation: "FIXED_AMOUNT", statutory: false, taxable: true, gross: true, net: true, ctc: true, payslip: true },
        { name: "Sales Commission", code: "SALES_COMMISSION", type: "EARNING", category: "BONUS", calculation: "FIXED_AMOUNT", statutory: false, taxable: true, gross: true, net: true, ctc: true, payslip: true },
        { name: "Attendance Bonus", code: "ATTENDANCE_BONUS", type: "EARNING", category: "BONUS", calculation: "FIXED_AMOUNT", statutory: false, taxable: true, gross: true, net: true, ctc: true, payslip: true },
        { name: "Performance Bonus", code: "PERFORMANCE_BONUS", type: "EARNING", category: "BONUS", calculation: "PERCENTAGE", statutory: false, taxable: true, gross: true, net: true, ctc: true, payslip: true },
        { name: "Joining Bonus", code: "JOINING_BONUS", type: "EARNING", category: "BONUS", calculation: "FIXED_AMOUNT", statutory: false, taxable: true, gross: true, net: true, ctc: true, payslip: true },
        { name: "Retention Bonus", code: "RETENTION_BONUS", type: "EARNING", category: "BONUS", calculation: "FIXED_AMOUNT", statutory: false, taxable: true, gross: true, net: true, ctc: true, payslip: true },
        { name: "Monthly Stipend", code: "MONTHLY_STIPEND", type: "EARNING", category: "ALLOWANCE", calculation: "FIXED_AMOUNT", statutory: false, taxable: true, gross: true, net: true, ctc: true, payslip: true },
        { name: "PF", code: "PF", type: "DEDUCTION", category: "PF", calculation: "PERCENTAGE", statutory: true, taxable: false, gross: false, net: true, ctc: false, payslip: true },
        { name: "ESI", code: "ESI", type: "DEDUCTION", category: "ESI", calculation: "PERCENTAGE", statutory: true, taxable: false, gross: false, net: true, ctc: false, payslip: true },
        { name: "Professional Tax", code: "PROFESSIONAL_TAX", type: "DEDUCTION", category: "PROFESSIONAL_TAX", calculation: "FIXED_AMOUNT", statutory: true, taxable: false, gross: false, net: true, ctc: false, payslip: true },
        { name: "TDS", code: "TDS", type: "DEDUCTION", category: "TDS", calculation: "FORMULA", statutory: true, taxable: false, gross: false, net: true, ctc: false, payslip: true },
        { name: "Labour Welfare Fund", code: "LWF", type: "DEDUCTION", category: "LWF", calculation: "FIXED_AMOUNT", statutory: true, taxable: false, gross: false, net: true, ctc: false, payslip: true },
        { name: "Loan Recovery", code: "LOAN_RECOVERY", type: "DEDUCTION", category: "LOAN", calculation: "FIXED_AMOUNT", statutory: false, taxable: false, gross: false, net: true, ctc: false, payslip: true },
        { name: "Salary Advance", code: "SALARY_ADVANCE", type: "DEDUCTION", category: "ADVANCE", calculation: "FIXED_AMOUNT", statutory: false, taxable: false, gross: false, net: true, ctc: false, payslip: true },
      ];
      ...
      for (const component of payrollComponents) {
        const existingComponent = page.locator("tr").filter({ hasText: component.name }).first();
        if (await existingComponent.isVisible().catch(() => false)) {
          console.log(`   [Skip] Payroll component already exists: "${component.name}"`);
          continue;
        }
      ...
      const percentage = (code, value) => ({ code, calculation: "PERCENTAGE", percentage: value });
      const formula = (code, expression) => ({ code, calculation: "FORMULA", formula: expression });
      const payrollStructures = [
        {
          name: "Executive Payroll", code: "EXECUTIVE_PAYROLL",
          appliesTo: "CEO, CTO, CFO, COO and Vice Presidents",
          components: [percentage("BASIC", 35), formula("HRA", "BASIC * 0.50"), formula("SPECIAL_ALLOWANCE", "GROSS - BASIC - HRA"), fixed("CAR_ALLOWANCE", 50000), fixed("FUEL_ALLOWANCE", 15000), fixed("DRIVER_ALLOWANCE", 20000), percentage("PERFORMANCE_BONUS", 20), percentage("PF", 12), fixed("PROFESSIONAL_TAX"), formula("TDS", "INCOME_TAX")],
        },
        {
          name: "Engineering Payroll", code: "ENGINEERING_PAYROLL",
          appliesTo: "Architects, Tech Leads, Engineers, QA and DevOps",
          components: [percentage("BASIC", 40), formula("HRA", "BASIC * 0.40"), formula("SPECIAL_ALLOWANCE", "GROSS - BASIC - HRA"), fixed("INTERNET_ALLOWANCE", 2000), fixed("FOOD_ALLOWANCE", 2200), fixed("MOBILE_ALLOWANCE", 1500), fixed("SHIFT_ALLOWANCE"), percentage("PERFORMANCE_BONUS", 10), fixed("OVERTIME"), percentage("PF", 12), fixed("PROFESSIONAL_TAX"), formula("TDS", "INCOME_TAX")],
        },
        {
          name: "Sales Payroll", code: "SALES_PAYROLL",
          appliesTo: "Sales Executives, Sales Managers and Regional Managers",
          components: [percentage("BASIC", 35), formula("HRA", "BASIC * 0.50"), fixed("TRAVEL_ALLOWANCE", 6000), fixed("FUEL_ALLOWANCE", 5000), fixed("MOBILE_ALLOWANCE", 2500), fixed("INCENTIVE"), fixed("SALES_COMMISSION"), percentage("PF", 12), fixed("PROFESSIONAL_TAX"), formula("TDS", "INCOME_TAX")],
        },
        {
          name: "HR & Administration Payroll", code: "HR_ADMIN_PAYROLL",
          appliesTo: "HR, Recruitment, Administration and Finance",
          components: [percentage("BASIC", 40), formula("HRA", "BASIC * 0.40"), fixed("CONVEYANCE", 2000), fixed("FOOD_ALLOWANCE", 2200), fixed("MEDICAL_ALLOWANCE", 1250), formula("SPECIAL_ALLOWANCE", "GROSS - BASIC - HRA"), percentage("PF", 12), fixed("PROFESSIONAL_TAX"), formula("TDS", "INCOME_TAX")],
        },
        {
          name: "Customer Support Payroll", code: "CUSTOMER_SUPPORT_PAYROLL",
          appliesTo: "Customer Success, Support Engineers and Helpdesk",
          components: [percentage("BASIC", 45), formula("HRA", "BASIC * 0.40"), fixed("NIGHT_SHIFT_ALLOWANCE"), fixed("INTERNET_ALLOWANCE", 1500), fixed("MEAL_ALLOWANCE", 2000), fixed("OVERTIME"), percentage("PF", 12), fixed("PROFESSIONAL_TAX"), formula("TDS", "INCOME_TAX")],
        },
        {
          name: "Operations Payroll", code: "OPERATIONS_PAYROLL",
          appliesTo: "Factory and Operations employees",
          components: [percentage("BASIC", 50), formula("HRA", "BASIC * 0.20"), fixed("SHIFT_ALLOWANCE", 3000), fixed("ATTENDANCE_BONUS", 2500), fixed("OVERTIME"), percentage("PF", 12), fixed("ESI"), fixed("PROFESSIONAL_TAX")],
        },
        {
          name: "Intern Payroll", code: "INTERN_PAYROLL",
          appliesTo: "Interns",
          components: [fixed("MONTHLY_STIPEND", 20000), fixed("INTERNET_ALLOWANCE", 1000), fixed("MEAL_ALLOWANCE", 1500)],
        },
      ];
      const componentNameByCode = new Map(payrollComponents.map((component) => [component.code, component.name]));
      for (const structure of payrollStructures) {
        const existingStructure = page.locator("tr").filter({ hasText: structure.name }).first();
        if (await existingStructure.isVisible().catch(() => false)) {
          console.log(`   [Skip] Payroll structure already exists: "${structure.name}"`);
          continue;
        }
        await slowClick('[data-testid="hr-payroll-structure-new"]', `New ${structure.name}`);
        payrollDialog = page.getByRole("dialog").filter({ hasText: "Payroll Structure" }).first();
        await payrollDialog.getByLabel("Structure Name").fill(structure.name);
        await payrollDialog.getByLabel("Structure Code").fill(`${structure.code}_${suffix}`);
        await payrollDialog.getByLabel("Frequency").selectOption("MONTHLY");
        await payrollDialog.getByLabel("Currency").fill("INR");
        await payrollDialog.getByLabel("Description").fill(`Applicable to ${structure.appliesTo}`);
        await payrollDialog.getByRole("button", { name: "Save", exact: true }).click();
        await payrollDialog.waitFor({ state: "hidden", timeout: 20000 });
        payrollRow = page.locator("tr").filter({ hasText: structure.name }).first();
        await payrollRow.waitFor({ state: "visible", timeout: 10000 });
        await payrollRow.locator('[data-testid^="hr-payroll-structure-build-"]').click();
        for (const mapping of structure.components) {
          await slowClick('[data-testid="hr-payroll-structure-builder-open-add"]', `Add ${mapping.code} to ${structure.name}`);
          const componentName = componentNameByCode.get(mapping.code);
          await page.locator('[data-testid="hr-payroll-structure-builder-component"]').selectOption({ label: `${componentName} (${mapping.code})` });
          await slowSelect('[data-testid="hr-payroll-structure-builder-calculation"]', mapping.calculation, `${mapping.code} Calculation`);
          payrollDialog = page.getByRole("dialog").filter({ hasText: "Add Component" }).first();
          if (mapping.calculation === "FIXED_AMOUNT") await payrollDialog.getByLabel("Default Amount").fill(String(mapping.amount ?? 0));
          if (mapping.calculation === "PERCENTAGE") await payrollDialog.getByLabel("Default Percentage").fill(String(mapping.percentage ?? 0));
          if (mapping.calculation === "FORMULA") await payrollDialog.getByLabel("Formula Expression").fill(mapping.formula ?? "");
          ...
          await slowClick('[data-testid="hr-payroll-structure-add-component"]', `Save ${mapping.code} Mapping`);
          await payrollDialog.waitFor({ state: "hidden", timeout: 20000 });
        }
        await page.keyboard.press("Escape").catch(() => { });
        await visitHr("/hr/payroll/structures", "PAYROLL STRUCTURES");
      ...
      const payrollEmployeeRow = page.locator("tr").filter({ has: nameCell }).first();
    ...
    async function runEditEmployeeProfile() {
      ...
      console.log(`   [Verified] Employee profile edited: "${primaryEmployeeName}"`);
    }
    ...
    async function runManageLoginAction() {
      ...
      const employeeRow = page.locator("tr").filter({ has: nameCell }).first();
    ...
    async function runSeparationActions() {
      ...
      const actOnExitRequest = async (outcome, sequence) => {
        ...
        const pendingRow = page.locator('[data-testid^="hr-separation-table-row-"]').filter({ hasText: primaryEmployeeName }).filter({ hasText: "Pending" }).first();
        ...
        await page.locator('[data-testid^="hr-separation-table-row-"]').filter({ hasText: primaryEmployeeName }).filter({ hasText: outcome }).first().waitFor({ state: "visible", timeout: 20000 });
