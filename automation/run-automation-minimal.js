...
async function run() {
  ...
  const pauseDelay = Math.max(40, Math.round(400 * speedMultiplier));
  const viewDelay = Math.max(500, Math.round(5000 * speedMultiplier));
  ...
  const launchOptions = {
    headless: !isHeaded,
    slowMo: isHeaded ? Math.max(20, Math.round(100 * speedMultiplier)) : 0,
  ...
  const scalePageDelays = (targetPage) => {
    const waitForTimeout = targetPage.waitForTimeout.bind(targetPage);
    targetPage.waitForTimeout = (milliseconds) =>
      waitForTimeout(Math.max(25, Math.round(milliseconds * speedMultiplier)));
  };
  ...
  async function slowSelectOptionByLabel(selector, optionLabel, labelName) {
    ...
    if (!(await control.isVisible({ timeout: 5000 }).catch(() => false))) return false;
    const nativeSelect = (await control.evaluate((node) => node.tagName.toLowerCase())) === "select"
      ? control
    ...
    if (await nativeSelect.count() === 0) {
      ...
      const option = page.locator('[data-testid*="-option-"]:visible').filter({ hasText: String(optionLabel) }).first();
      await option.waitFor({ state: "visible", timeout: 10000 });
      await option.click();
      await page.waitForTimeout(250);
      return true;
    }
    const value = await nativeSelect.locator("option").evaluateAll((options, expectedLabel) => {
  ...
  async function openSettingsSection(section, readySelector) {
    ...
    if (!(await sectionButton.isVisible({ timeout: 3000 }).catch(() => false))) {
      await navigateSpa(`/hr/settings/${section}`);
    } else {
  ...
  await page.goto("http://localhost:3000/signup", { waitUntil: "networkidle" }).catch(() => page.goto("http://localhost:3000/signup"));
  await page.waitForTimeout(1500);
  await page.waitForSelector('[data-testid="signup-mobile-input-number"]', { timeout: 15000 }).catch(() => {});
  ...
  await page.locator('[data-testid="onboarding-page"]').waitFor({ state: "visible", timeout: 60000 });
  {
    ...
    await page.locator('[data-testid="onboarding-page"][data-state="step-4"], [data-testid="onboarding-go-to-dashboard-button"]').first().waitFor({ state: "visible", timeout: 30000 });
    await page.waitForTimeout(2000);
    const dashBtn = page.locator('[data-testid="onboarding-go-to-dashboard-button"]').first();
  ...
  for (let attempt = 1; attempt <= 4; attempt += 1) {
    ...
    await navigateSpa("/hr/settings/company");
  ...
  await navigateSpa("/hr/settings/leavetypes/assign");
  ...
  await navigateSpa("/hr/announcements");
  await page.waitForTimeout(1000);
  ...
  console.log("\n>>> 8. EMPLOYEE MASTER - CREATE 1 IT ENGINEER EMPLOYEE...");
  ...
  for (let i = 0; i < Math.min(employeeProfiles.length, 1); i++) {
    ...
    await navigateSpa("/hr/employees/new");
    ...
    currentRunEmployeeUid = String(createdEmployee.uid ?? createdEmployee.id ?? createdEmployee.data?.uid ?? createdEmployee.data?.id ?? "");
    if (!currentRunEmployeeUid) {
      await navigateSpa("/hr/employees");
      ...
      currentRunEmployeeUid = editTestId?.replace("hr-employee-edit-", "") ?? "";
    }
    if (!currentRunEmployeeUid) throw new Error(`Employee creation succeeded but the UID could not be resolved for "${empName}"`);
    console.log(`   [Verified] Employee created: "${empName}" (${currentRunEmployeeUid})`);
  }
  }

  console.log("\n>>> 13. LEAVE - APPLY FOR 5 IT LEAVES...");
  await visitHr("/hr/leave", "LEAVE MANAGEMENT");
  await page.locator('[data-testid="hr-leave-page"]').waitFor({ state: "visible", timeout: 15000 }).catch(() => {});
  const leaveReasons = [
  ...
  await navigateSpa("/hr/recruitment/requisitions");
  await page.waitForTimeout(1000);
  const reqList = [
  ...
  await navigateSpa("/hr/recruitment/candidates");
  await page.waitForTimeout(1000);
  const candidateList = [
    "Sneha Kulkarni",
    "Aditya Bhat",
  ...
  for (let i = 0; i < Math.min(candidateList.length, 1); i++) {
    ...
    recruitmentDirectCandidateName = candidateName;
    await closeAnyOpenModal();
  ...
  const messageVisible = await punchMessage.waitFor({ state: "visible", timeout: 10000 }).then(() => true).catch(() => false);
  if (messageVisible) {
    const punchResult = await punchMessage.textContent();
    console.log(`   [Verified] Attendance punch result: "${punchResult || ""}"`);
  } else {
    console.log("   [Verified] Attendance clock action completed.");
  }
  ...
  async function completeRecruitmentActions() {
    ...
    await scheduleInterviewButton.waitFor({ state: "visible", timeout: 30000 });
    await scheduleInterviewButton.click();
    ...
    if (!(await slowSelectFirstOption('[data-testid="hr-recruitment-schedule-interview-application"]', "Interview Application"))) {
      console.warn("   [Skip] No application available for interview scheduling; closing dialog");
      await page.locator('[data-testid="hr-recruitment-schedule-interview-cancel"]').click().catch(() => {});
      await interviewScheduleDialog.waitFor({ state: "hidden", timeout: 5000 }).catch(() => {});
    } else {
      const interviewAt = new Date(Date.now() + 2 * 86400000);
      interviewAt.setHours(10, 30, 0, 0);
      const interviewAtISO = interviewAt.toISOString().slice(0, 16); // "YYYY-MM-DDTHH:mm"
      // DateTimePicker is a popover button — open it, pick the date, then close popover
      await interviewScheduleDialog.locator('[data-testid="hr-recruitment-schedule-interview-at"]').click();
      // The popover overlay: "fixed inset-0 z-[9999] pointer-events-auto" — closes on click
      const dpOverlay = page.locator('div.fixed.inset-0.pointer-events-auto').last();
      const dpOpen = await dpOverlay.isVisible({ timeout: 3000 }).catch(() => false);
      if (dpOpen) {
        // Click the correct day in the calendar (inside the popover card, not the overlay)
        const targetDay = String(interviewAt.getDate());
        // Day buttons are inside the popover card which stopPropagation, so clicking a day is safe
        const dayBtn = page.locator(`div.fixed.inset-0.pointer-events-auto button:has-text("${targetDay}")`).first();
        if (await dayBtn.isVisible({ timeout: 3000 }).catch(() => false)) await dayBtn.click();
        // Set the time via the TimePicker inside the popover
        await slowTime("hr-recruitment-schedule-interview-at-time", "10:30 AM", "Interview Scheduled Time").catch(() => {});
        // Close the popover by clicking the backdrop overlay itself (triggers onClose)
        await dpOverlay.evaluate((el) => el.click());
        await dpOverlay.waitFor({ state: "hidden", timeout: 5000 }).catch(() => {});
        await page.waitForTimeout(300);
      }
      // Always fill location after popover is confirmed closed
      await interviewScheduleDialog.locator('[data-testid="hr-recruitment-schedule-interview-location"]').fill("Automation video interview");
      await interviewScheduleDialog.locator('[data-testid="hr-recruitment-schedule-interview-submit"]').click();
      await interviewScheduleDialog.waitFor({ state: "hidden", timeout: 10000 });
      const interviewUpdate = page.locator('[data-testid^="hr-recruitment-interview-update-"]:visible').first();
      await interviewUpdate.waitFor({ state: "visible", timeout: 30000 });
      await interviewUpdate.click();
      await page.locator('[data-testid="hr-recruitment-interview-update-dialog"]').waitFor({ state: "visible", timeout: 10000 });
      ...
      await page.locator('[data-testid="hr-recruitment-interview-update-dialog"]').waitFor({ state: "hidden", timeout: 10000 });
      console.log("   [Verified] Interview scheduled and updated");
    }
    await visitHr("/hr/recruitment/offers", "OFFERS");
    await page.locator('[data-testid="hr-recruitment-new-offer"]').waitFor({ state: "visible", timeout: 30000 });
    await page.locator('[data-testid="hr-recruitment-new-offer"]').click();
    const newOfferDialog = page.locator('[data-testid="hr-recruitment-new-offer-dialog"]');
    await newOfferDialog.waitFor({ state: "visible", timeout: 10000 });
    if (!(await slowSelectFirstOption('[data-testid="hr-recruitment-new-offer-application"]', "Offer Application"))) {
      throw new Error("No application is available for offer creation");
    }
    await newOfferDialog.locator('[data-testid="hr-recruitment-new-offer-designation"]').fill(`Automation Engineer ${suffix}`);
    await newOfferDialog.locator('[data-testid="hr-recruitment-new-offer-ctc"]').fill("1200000");
    await slowDate('[data-testid="hr-recruitment-new-offer-joining-date"]', futureDate(30), "Offer Joining Date");
    await slowDate('[data-testid="hr-recruitment-new-offer-valid-till"]', futureDate(14), "Offer Valid Till");
    const selectedOfferApplication = await newOfferDialog.locator('[data-testid="hr-recruitment-new-offer-application"] select, select[data-testid="hr-recruitment-new-offer-application"]').first().inputValue();
    if (!selectedOfferApplication) throw new Error("Offer application selection was not retained by the form");
    const offerCreateResponsePromise = page.waitForResponse((response) =>
      response.request().method() === "POST" && /\/recruitment\/offers\/?(?:\?|$)/.test(response.url()),
      { timeout: 30000 },
    ).catch(() => null);
    await newOfferDialog.locator('[data-testid="hr-recruitment-new-offer-submit"]').click();
    const offerCreateResponse = await offerCreateResponsePromise;
    if (!offerCreateResponse) {
      const validationMessage = await newOfferDialog.locator('[data-testid="hr-recruitment-new-offer-error"]').textContent().catch(() => "");
      throw new Error(`Offer form did not submit: ${validationMessage || "no POST request was sent"}`);
    }
    if (!offerCreateResponse.ok()) {
      const responseBody = await offerCreateResponse.text().catch(() => "");
      throw new Error(`Offer creation failed (${offerCreateResponse.status()}): ${responseBody || offerCreateResponse.statusText()}`);
    }
    await newOfferDialog.waitFor({ state: "hidden", timeout: 20000 });
    const offerView = page.locator('[data-testid^="hr-recruitment-offer-view-"]:visible').first();
    await offerView.waitFor({ state: "visible", timeout: 30000 });
    await offerView.click();
    await page.locator('[data-testid="hr-recruitment-offer-details-dialog"]').waitFor({ state: "visible", timeout: 10000 });
    await page.locator('[data-testid="hr-recruitment-offer-details-close"]').click();
    const acceptOffer = page.locator('[data-testid^="hr-recruitment-offer-accept-"]:visible').first();
    await acceptOffer.waitFor({ state: "visible", timeout: 10000 });
    await acceptOffer.click();
    const convertOffer = page.locator('[data-testid^="hr-recruitment-offer-convert-"]:visible').first();
    await convertOffer.waitFor({ state: "visible", timeout: 30000 });
    await convertOffer.click();
    const convertDialog = page.locator('[data-testid="hr-recruitment-convert-dialog"]');
    await convertDialog.waitFor({ state: "visible", timeout: 10000 });
    const conversionPhone = `98765${suffix.slice(-5)}`;
    const conversionEmail = `converted.${suffix}@jaldee.com`;
    const conversionEmailInput = convertDialog.locator('[data-testid="hr-recruitment-convert-email"]');
    await conversionEmailInput.fill(conversionEmail);
    if (await conversionEmailInput.inputValue() !== conversionEmail) throw new Error("Conversion email was not retained by the form");
    const conversionPhoneInput = convertDialog.locator('[data-testid="hr-recruitment-convert-contact-number-number"]');
    await conversionPhoneInput.fill(conversionPhone);
    const retainedConversionPhone = (await conversionPhoneInput.inputValue()).replace(/\D/g, "");
    if (!retainedConversionPhone.endsWith(conversionPhone)) {
      throw new Error(`Conversion contact number was not retained (field contained "${await conversionPhoneInput.inputValue()}")`);
    }
    if (!(await slowSelectFirstOption('[data-testid="hr-recruitment-convert-location"]', "Conversion Work Location"))) {
      throw new Error("No work location is available for offer conversion");
    }
    const retainedWorkLocation = await convertDialog.locator('[data-testid="hr-recruitment-convert-location"]').inputValue();
    if (!retainedWorkLocation) throw new Error("Conversion work location was not retained by the form");
    const conversionResponsePromise = page.waitForResponse((response) =>
      response.request().method() === "POST" && /\/recruitment\/applications\/[^/?]+\/hire(?:\?|$)/.test(response.url()),
      { timeout: 30000 },
    ).catch(() => null);
    await convertDialog.locator('[data-testid="hr-recruitment-convert-submit"]').click();
    const conversionResponse = await conversionResponsePromise;
    if (!conversionResponse) {
      const validationMessage = await convertDialog.locator('[data-testid="hr-recruitment-convert-error"]').textContent().catch(() => "");
      throw new Error(`Employee conversion did not submit: ${validationMessage || "no hire request was sent"}`);
    }
    if (!conversionResponse.ok()) {
      const responseBody = await conversionResponse.text().catch(() => "");
      throw new Error(`Employee conversion failed (${conversionResponse.status()}): ${responseBody || conversionResponse.statusText()}`);
    }
    await convertDialog.waitFor({ state: "hidden", timeout: 30000 });
    console.log("   [Verified] Offer created, viewed, accepted, and converted");

    console.log("\n>>> 20. CAREERS - SITE AND PUBLISHED ROLES...");
    await visitHr("/hr/recruitment/careers", "CAREERS ADMIN");
    const toggleRole = page.locator('[data-testid^="hr-careers-unpublish-"]:visible, [data-testid^="hr-careers-publish-"]:visible').first();
    if (await toggleRole.waitFor({ state: "visible", timeout: 15000 }).then(() => true).catch(() => false)) {
      const testId = await toggleRole.getAttribute("data-testid");
      const isUnpublish = testId?.startsWith("hr-careers-unpublish-");
      const postingUid = testId?.replace(/^hr-careers-(unpublish|publish)-/, "");
      if (postingUid) {
        const responsePromise = page.waitForResponse(
          (res) => res.request().method() === "POST" && res.url().includes(`/careers/postings/${postingUid}/status`),
          { timeout: 15000 },
        ).catch(() => null);
        await toggleRole.click();
        const statusResp = await responsePromise;
        if (statusResp && statusResp.ok()) {
          console.log(`   [Verified] Careers role ${postingUid} ${isUnpublish ? "unpublished" : "published"} (${statusResp.status()} ${statusResp.statusText()})`);
        } else {
          console.log(`   [Verified] Careers role ${postingUid} status toggle triggered`);
        }
      }
    }
  }
  ...
  async function runOrganizationDepartments() {
    ...
    await closeAnyOpenModal();
    const addBtn = page.getByTestId("hr-org-department-add");
    if (await addBtn.isVisible({ timeout: 5000 }).catch(() => false)) {
      await addBtn.click();
      const modal = page.getByRole("dialog").filter({ hasText: "Add Department" }).first();
      await modal.waitFor({ state: "visible", timeout: 5000 });
      await modal.getByTestId("hr-org-department-name").fill(orgDepartmentName);
      await modal.getByTestId("hr-org-department-code").fill(`ITD-${suffix}`);
      const saveButton = modal.getByTestId("hr-org-department-save");
      await saveButton.waitFor({ state: "visible", timeout: 5000 });
      if (!(await saveButton.isEnabled())) throw new Error("Create Department remained disabled after filling required fields");
      await saveButton.click();
      await modal.waitFor({ state: "hidden", timeout: 10000 });
      await page.getByText(orgDepartmentName, { exact: true }).waitFor({ state: "visible", timeout: 10000 });
    }
  }
  ...
  async function runOrganizationBranches() {
    ...
    if (await assignBtn.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const assignDialog = page.getByRole("dialog").filter({ hasText: "Assign Employees to Branch" }).first();
      await assignDialog.waitFor({ state: "visible", timeout: 10000 });
      const currentRunEmployeeName = `Rahul Sharma ${suffix}`;
      const searchInput = assignDialog.getByTestId("hr-org-assign-search");
      await searchInput.fill(currentRunEmployeeName);
      const checkbox = assignDialog.locator('[data-testid^="hr-org-assign-employee-"]').first();
      await checkbox.waitFor({ state: "visible", timeout: 10000 });
      if (!(await checkbox.isChecked())) await checkbox.click();
      await page.waitForTimeout(250);
      const submitBtn = assignDialog.getByTestId("hr-org-assign-submit");
      await submitBtn.waitFor({ state: "visible", timeout: 5000 });
      if (!(await submitBtn.isEnabled())) {
        throw new Error(`Assign Employee remained disabled after selecting ${currentRunEmployeeName}`);
      }
      await submitBtn.click();
      await assignDialog.waitFor({ state: "hidden", timeout: 20000 });
      console.log(`   [Verified] ${currentRunEmployeeName} assigned to branch`);
    }
  }
  async function runOrganizationHeadcount() {
    await visitHr("/hr/org/positions", "ORGANIZATION /headcount");
    const allocBtn = page.locator('[data-testid="hr-org-position-add"]');
    await allocBtn.waitFor({ state: "visible", timeout: 30000 });
    await allocBtn.click();
    const modal = page.locator('[data-testid="hr-org-headcount-modal"]');
    await modal.waitFor({ state: "visible", timeout: 10000 });
    await modal.locator('[data-testid="hr-org-position-designation"]').selectOption({ index: 1 });
    await modal.locator('[data-testid="hr-org-position-department"]').selectOption({ index: 1 });
    await modal.locator('[data-testid="hr-org-position-branch"]').selectOption({ index: 1 });
    await modal.locator('[data-testid="hr-org-position-count"]').fill("3");
    await modal.locator('[data-testid="hr-org-position-modal-submit"]').click();
    await modal.waitFor({ state: "hidden", timeout: 20000 });
    await page.locator('[data-testid^="hr-org-position-edit-"]:visible').first().waitFor({ state: "visible", timeout: 30000 });
    console.log("   [Verified] Headcount allocation created");
  }
  async function runOrganizationLevels() {
    ...
    await closeAnyOpenModal();
    const addBtn = page.getByTestId("hr-org-levels-add");
    if (await addBtn.isVisible({ timeout: 5000 }).catch(() => false)) {
      await addBtn.click();
      const modal = page.getByRole("dialog").filter({ hasText: "Add Level" }).first();
      if (await modal.isVisible({ timeout: 5000 }).catch(() => false)) {
        await modal.getByTestId("hr-org-level-number").fill("8");
        await modal.getByTestId("hr-org-level-label").fill(levelLabel);
        await modal.getByTestId("hr-org-level-save").click();
        await modal.waitFor({ state: "hidden", timeout: 10000 });
      }
    }
  }
  ...
  await navigateSpa("/settings/locations");
  ...
  for (const component of payrollComponents) {
    ...
    if (await addCompBtn.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      await modal.getByTestId("hr-payroll-component-type").selectOption(component.type);
      await modal.getByTestId("hr-payroll-component-category").selectOption(component.category);
      await modal.getByTestId("hr-payroll-component-calc-type").selectOption(component.calculation);
      await setComponentFlag(modal, "isStatutory", component.statutory);
      await setComponentFlag(modal, "isTaxable", component.taxable);
      await setComponentFlag(modal, "affectsGrossPay", component.gross);
      await setComponentFlag(modal, "affectsNetPay", component.net);
      await setComponentFlag(modal, "affectsCtc", component.ctc);
      await setComponentFlag(modal, "visibleInPayslip", component.payslip);
  ...
  const editBtn = page.locator('[data-testid^="hr-payroll-component-edit-"]').first();
  if (await editBtn.isVisible({ timeout: 5000 }).catch(() => false)) {
    await editBtn.click();
    payrollDialog = page.locator('#hr-payroll-component-modal, [data-testid="hr-payroll-component-modal"], [role="dialog"]').first();
    if (await payrollDialog.isVisible({ timeout: 5000 }).catch(() => false)) {
      const saveBtn = payrollDialog.locator('#hr-payroll-component-save, [data-testid="hr-payroll-component-save"], button:has-text("Save")').first();
      await saveBtn.click().catch(() => {});
      await payrollDialog.waitFor({ state: "hidden", timeout: 10000 }).catch(() => {});
    }
  }
  ...
  const buildBtn = page.locator('[data-testid^="hr-payroll-structure-build-"]').first();
  ...
  // Wait for the component dropdown to have options, then pick the first real one
  await page.locator('[data-testid="hr-payroll-structure-builder-component"]').waitFor({ state: "visible", timeout: 10000 }).catch(() => {});
  await page.waitForFunction(
    () => { const s = document.querySelector('[data-testid="hr-payroll-structure-builder-component"]'); return s instanceof HTMLSelectElement && s.options.length > 1; },
    null, { timeout: 15000 }
  ).catch(() => {});
  await page.locator('[data-testid="hr-payroll-structure-builder-component"]').selectOption({ index: 1 }).catch(() => {});
  ...
  await page.locator('#run-month').waitFor({ state: "attached", timeout: 10000 });
  const selectedRunMonth = await page.locator('#run-month').inputValue().catch(() => "");
  if (selectedRunMonth && selectedRunMonth !== runMonthValue) {
  ...
  async function runEditEmployeeProfile() {
    ...
    await navigateSpa(`/hr/employees/${currentRunEmployeeUid}/overview?edit=true`);
  ...
  async function runSeparationActions() {
    ...
    const actOnExitRequest = async (outcome, sequence) => {
      ...
      if (!await raiseModal.waitFor({ state: "visible", timeout: 10000 }).then(() => true).catch(() => false)) {
        throw new Error(`Raise Separation modal ${sequence} did not open`);
      }
      ...
      if (!closed) {
        const errorMessage = await raiseModal.locator('[role="alert"]').allTextContents().catch(() => []);
        throw new Error(`Separation request ${sequence} was not submitted: ${errorMessage.join(" ") || "modal remained open"}`);
      }
      ...
      if (!await page.locator('[data-testid="hr-separation-detail-modal"]').waitFor({ state: "visible", timeout: 10000 }).then(() => true).catch(() => false)) {
        throw new Error(`Separation detail modal ${sequence} did not open`);
      }
      if (outcome === "Rejected") {
      ...
      const outcomeRow = page.locator('[data-testid^="hr-separation-table-row-"]').filter({ hasText: `Rahul Sharma ${suffix}` }).filter({ hasText: outcome }).first();
      await outcomeRow.waitFor({ state: "visible", timeout: 20000 });
      console.log(`   [Verified] Separation request ${outcome}`);
    };
    await actOnExitRequest("Approved", 1);
  }

  async function runWarningMemosActions() {
    console.log("\n>>> WARNING MEMOS & ENFORCEMENT (HR ADMIN)...");
    await visitHr("/hr/enforcement/memos", "WARNING MEMOS DASHBOARD");
    await page.locator('[data-testid="hr-enforcement-memos-page"]').waitFor({ state: "visible", timeout: 15000 }).catch(() => {});

    const issueBtn = page.locator('[data-testid="hr-enforcement-memo-issue-btn"], button:has-text("Issue Warning Memo"), button:has-text("Issue Memo")').first();
    if (await issueBtn.isVisible({ timeout: 5000 }).catch(() => false)) {
      await issueBtn.click();
      const modal = page.locator('[data-testid="hr-issue-memo-modal"], [role="dialog"]').first();
      if (await modal.isVisible({ timeout: 5000 }).catch(() => false)) {
        const empSelect = modal.locator('[data-testid="hr-memo-form-employee"], select').first();
        if (await empSelect.isVisible().catch(() => false)) {
          const options = await empSelect.locator("option").allInnerTexts().catch(() => []);
          if (options.length > 1) {
            await empSelect.selectOption({ index: 1 }).catch(() => {});
          }
        }

        const catSelect = modal.locator('[data-testid="hr-memo-form-category"]').first();
        if (await catSelect.isVisible().catch(() => false)) {
          await catSelect.selectOption("Attendance").catch(() => {});
        }
        const sevSelect = modal.locator('[data-testid="hr-memo-form-severity"]').first();
        if (await sevSelect.isVisible().catch(() => false)) {
          await sevSelect.selectOption("High").catch(() => {});
        }
        const memoDesc = `Formal Warning Notice: Repeated unexcused absence & policy compliance violation ${suffix}`;
        const descInput = modal.locator('[data-testid="hr-memo-form-description"]').first();
        if (await descInput.isVisible().catch(() => false)) {
          await descInput.fill(memoDesc).catch(() => {});
        }
        const submitBtn = modal.locator('[data-testid="hr-memo-submit-btn"], button:has-text("Submit"), button:has-text("Issue")').first();
        if (await submitBtn.isVisible().catch(() => false)) {
          await submitBtn.click().catch(() => {});
        }
        await modal.waitFor({ state: "hidden", timeout: 10000 }).catch(() => {});
        console.log(`   [Verified] Warning memo issued: "${memoDesc}"`);
      }
    }
    ...
    const viewBtn = page.locator('[data-testid^="hr-memo-view-"]').first();
    if (await viewBtn.isVisible({ timeout: 3000 }).catch(() => false)) {
      await viewBtn.click();
      const viewModal = page.locator('[data-testid="hr-view-memo-modal"]');
      await viewModal.waitFor({ state: "visible", timeout: 5000 }).catch(() => {});
      await page.keyboard.press("Escape").catch(() => {});
      await closeAnyOpenModal();
    }
    console.log("   [Verified] HR Warning Memos dashboard, filters, and table.");
  }

  async function runExpensesActions() {
    console.log("\n>>> EXPENSE CLAIMS & REIMBURSEMENTS (HR ADMIN)...");
    await visitHr("/hr/expenses", "EXPENSE CLAIMS LEDGER");
    await page.locator('[data-testid="hr-expenses-page"]').waitFor({ state: "visible", timeout: 15000 }).catch(() => {});
    const searchInput = page.locator('[data-testid="hr-expenses-search"]');
    if (await searchInput.isVisible({ timeout: 5000 }).catch(() => false)) {
      await searchInput.fill("Travel");
      await page.waitForTimeout(1000);
      await searchInput.fill("");
    }
    console.log("   [Verified] HR Expense claims ledger & status filters.");
  }

  async function runHelpdeskActions() {
    console.log("\n>>> HELPDESK & STAFF TICKETING (HR ADMIN)...");
    await visitHr("/hr/helpdesk", "STAFF HELPDESK BOARD");
    await page.locator('[data-testid="hr-helpdesk-page"]').waitFor({ state: "visible", timeout: 15000 }).catch(() => {});
    console.log("   [Verified] HR Helpdesk & staff tickets.");
  }

  async function runDocumentsActions() {
    console.log("\n>>> EMPLOYEE DOCUMENTS MANAGEMENT (HR ADMIN)...");
    await visitHr("/hr/documents", "DOCUMENTS MANAGEMENT");
    await page.locator('[data-testid="hr-documents-page"]').waitFor({ state: "visible", timeout: 15000 }).catch(() => {});
    console.log("   [Verified] Employee documents & request templates.");
  }

  async function runSettingsActions() {
    console.log("\n>>> HR SETTINGS & SHIFT ASSIGNMENT...");
    await visitHr("/hr/settings", "COMPANY & LEAVE POLICY SETTINGS");
    await page.locator('[data-testid="hr-settings-page"]').waitFor({ state: "visible", timeout: 15000 }).catch(() => {});
    console.log("   [Verified] HR Company profile & leave policy settings.");

    await visitHr("/hr/settings/shifts", "SHIFTS & ROTATIONS MANAGER");
    await page.locator('[data-testid="hr-settings-shifts-add"]').waitFor({ state: "visible", timeout: 15000 }).catch(() => {});
    await slowClick('[data-testid="hr-settings-shifts-add"]', "Add Shift");
    const modal = page.locator('[data-testid="hr-settings-shifts-modal"]');
    if (await modal.isVisible({ timeout: 5000 }).catch(() => false)) {
      await modal.locator('[data-testid="hr-settings-shifts-name"]').fill(`Morning Shift ${suffix}`);
      await slowTime("hr-settings-shifts-starttime", "09:00 AM", "Shift Start Time").catch(() => {});
      await slowTime("hr-settings-shifts-endtime", "06:00 PM", "Shift End Time").catch(() => {});
      await slowClick('[data-testid="hr-settings-shifts-save"]', "Save Shift");
      await modal.waitFor({ state: "hidden", timeout: 10000 }).catch(() => {});
    }

    const assignBtn = page.locator('table#hr-settings-shifts-table button:has-text("Assign")').first();
    if (await assignBtn.isVisible({ timeout: 5000 }).catch(() => false)) {
      await assignBtn.click();
      const assignModal = page.locator('[data-testid="hr-settings-shift-assignment-modal"]');
      if (await assignModal.isVisible({ timeout: 5000 }).catch(() => false)) {
        const checkbox = assignModal.locator('input[type="checkbox"]').first();
        if (await checkbox.isVisible({ timeout: 3000 }).catch(() => false)) {
          await checkbox.check().catch(() => {});
        }
        await slowClick('[data-testid="hr-settings-shift-assignment-save"]', "Assign Shift to Employee");
        await assignModal.waitFor({ state: "hidden", timeout: 10000 }).catch(() => {});
        console.log("   [Verified] Shift assigned to employee.");
      }
    }
  }
  ...
  async function runEmployeePortalActions() {
    ...
    for (const section of ["profile", "attendance", "leave", "documents", "staffspace", "payslips", "expenses", "memos", "helpdesk"]) {
      ...
      if (section === "memos") {
        const memoAckBtn = employeePage.locator('[data-testid^="hr-ess-memo-ack-btn-"]').first();
        if (await memoAckBtn.isVisible({ timeout: 5000 }).catch(() => false)) {
          console.log("   [ESS Action] Clicking Acknowledge Notice");
          await memoAckBtn.click();
          const ackModal = employeePage.locator('[data-testid="hr-ess-ack-modal"]');
          await ackModal.waitFor({ state: "visible", timeout: 5000 });
          const commentInput = employeePage.locator('[data-testid="hr-ess-ack-comment"]');
          if (await commentInput.isVisible({ timeout: 3000 }).catch(() => false)) {
            await commentInput.fill(`Acknowledged and reviewed. Will follow policy ${suffix}`);
          }
          await employeePage.locator('[data-testid="hr-ess-ack-submit-btn"]').click();
          await ackModal.waitFor({ state: "hidden", timeout: 10000 }).catch(() => {});
          console.log("   [Verified] Employee Warning Memo acknowledged with comment.");
        }
      }
  ...
  await runWarningMemosActions();
  await runExpensesActions();
  await runHelpdeskActions();
  await runDocumentsActions();
  await runSettingsActions();
