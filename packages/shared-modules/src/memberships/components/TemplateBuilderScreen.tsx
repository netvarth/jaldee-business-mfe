...
function AddItemModal({
  ...
  const isObject = state?.kind === "object" || state?.kind === "editObject";

  if (!isOpen) return null;

  return (
    <div data-testid="jaldee-membership-template-builder-add-item-modal" className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4 backdrop-blur-sm">
      <div className="flex max-h-[85vh] w-full max-w-4xl flex-col rounded-xl border border-slate-200 bg-white shadow-xl">
        <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4">
          ...
          <Button id="jaldee-membership-template-builder-add-item-close-button" data-testid="jaldee-membership-template-builder-add-item-close-button" size="sm" variant="ghost" icon={<X size={16} />} onClick={onClose} aria-label="Close modal" />
        </div>

        <div className="overflow-y-auto p-6">
          {isObject ? (
            <div className="grid gap-3 md:grid-cols-3">
              <Input id="jaldee-membership-template-builder-add-object-key-input" data-testid="jaldee-membership-template-builder-add-object-key-input" label="Section data key" value={objectDraft.key} onChange={(event) => onObjectChange({ key: event.target.value })} placeholder="contactDetails" />
              <Input id="jaldee-membership-template-builder-add-object-title-input" data-testid="jaldee-membership-template-builder-add-object-title-input" label="Section label" value={objectDraft.title} onChange={(event) => onObjectChange({ title: event.target.value })} placeholder="Contact details" />
              <Input id="jaldee-membership-template-builder-add-object-description-input" data-testid="jaldee-membership-template-builder-add-object-description-input" label="Section note" value={objectDraft.description} onChange={(event) => onObjectChange({ description: event.target.value })} placeholder="Groups related inputs" />
              <div className="mt-7 flex h-9 items-center">
                <Checkbox
                  data-testid="jaldee-membership-template-builder-add-object-required-checkbox"
            ...
            <FieldEditor field={fieldDraft} onChange={onFieldChange} testIdPrefix="jaldee-membership-template-builder-add-field" />
          )}
        </div>

        <div className="flex shrink-0 justify-end gap-2 border-t border-slate-200 px-4 py-3">
          <Button id="jaldee-membership-template-builder-add-item-cancel-button" data-testid="jaldee-membership-template-builder-add-item-cancel-button" variant="outline" onClick={onClose}>
            Cancel
          </Button>
          <Button id="jaldee-membership-template-builder-add-item-confirm-button" data-testid="jaldee-membership-template-builder-add-item-confirm-button" icon={isEdit ? <Save size={16} /> : <Plus size={16} />} onClick={onConfirm}>
            {isEdit ? "Save" : "Add"}
          </Button>
        </div>
      </div>
...
export function TemplateBuilderScreen({ initialTemplateUid, onSave }: TemplateBuilderScreenProps) {
  const navigate = useSharedNavigate();
  const { account, basePath } = useSharedModulesContext();
  const isEditMode = Boolean(initialTemplateUid);
  ...
  useEffect(() => {
    if (initialTemplateUid) {
      setIsLoading(true);
      membershipTemplateService.detail(initialTemplateUid)
        .then((template) => {
          setTemplateName(template.name);
          setItems(template.templateSchema ? schemaToItems(template.templateSchema) : []);
        })
    ...
    setItems((current) =>
      current.map((item) =>
        item.kind === "object" && item.object.id === objectId
          ? makeObjectItem({
              ...item.object,
              fields: item.object.fields.filter((field) => field.id !== fieldId),
            })
          : item,
      ),
    );
  ...
  function confirmAddModal() {
    ...
    if (addModal.kind === "field") {
      ...
      setItems((current) => [...current, makeFieldItem({ ...fieldDraft, key: trimmedKey })]);
      ...
      setItems((current) =>
        current.map((item) =>
          item.kind === "field" && item.id === addModal.itemId
            ? makeFieldItem({ ...fieldDraft, key: trimmedKey })
            : item,
        ),
      );
    } else if (addModal.kind === "object") {
      const trimmedKey = objectDraft.key.trim();
      if (!trimmedKey) return;
      setItems((current) => [...current, makeObjectItem({ ...objectDraft, key: trimmedKey })]);
      ...
      setItems((current) =>
        current.map((item) =>
          item.kind === "object" && item.id === addModal.itemId
            ? makeObjectItem({ ...objectDraft, key: trimmedKey, fields: item.object.fields })
            : item,
        ),
      );
      ...
      setItems((current) =>
        current.map((item) =>
          item.kind === "object" && item.object.id === addModal.objectId
            ? makeObjectItem({
                ...item.object,
                fields: [...item.object.fields, { ...fieldDraft, key: trimmedKey }],
              })
            : item,
        ),
      );
    } else if (addModal.kind === "editObjectField") {
      const trimmedKey = fieldDraft.key.trim();
      if (!trimmedKey) return;
      setItems((current) =>
        current.map((item) =>
          item.kind === "object" && item.object.id === addModal.objectId
            ? makeObjectItem({
                ...
                fields: item.object.fields.map((field) =>
                  field.id === addModal.fieldId ? { ...fieldDraft, key: trimmedKey } : field,
                ),
              })
            : item,
        ),
  ...
  async function handleSaveTemplate() {
    ...
    try {
      ...
      if (isEditMode && initialTemplateUid) {
        await membershipTemplateService.update(
          initialTemplateUid,
          { ...requestPayload, uid: initialTemplateUid },
          fields,
          templateSchema,
        );
      } else {
        await membershipTemplateService.create(requestPayload, fields, templateSchema);
      }

      await onSave?.();
      setStatus(isEditMode ? "Template updated." : "Template created.");
  ...
  if (isLoading) {
    return (
      <div className="h-full bg-slate-50 p-4 sm:p-6 md:p-8 flex items-center justify-center">
        <div className="text-slate-500 animate-pulse text-sm">Loading template details...</div>
      </div>
    );
  }

  return (
    <div data-testid="jaldee-membership-template-builder-page" data-state={isSaving ? "loading" : status ? "status" : "idle"} className="h-full overflow-y-auto bg-slate-50 p-4 sm:p-6 md:p-8 pb-24">
      <PageHeader
        title={isEditMode ? "Edit Membership Template" : "Membership Template Builder"}
        subtitle="Build membership intake template JSON in the exact order required."
        back={{ label: "Templates", href: `${basePath}/templates` }}
        onNavigate={navigate}
        actions={
          <div className="flex items-center gap-2">
            <Button id="jaldee-membership-template-builder-cancel-button" data-testid="jaldee-membership-template-builder-cancel-button" variant="outline" onClick={() => navigate(`${basePath}/templates`)}>
              Cancel
            </Button>
            <Button id="jaldee-membership-template-builder-save-button" data-testid="jaldee-membership-template-builder-save-button" icon={<Save size={16} />} loading={isSaving} onClick={handleSaveTemplate}>
      ...
      <div className="grid gap-4 xl:grid-cols-[1.25fr,0.75fr]">
        <div className="flex flex-col gap-4">
          <SectionCard title="Template">
            <div className="grid gap-3 md:grid-cols-2">
              <Input id="jaldee-membership-template-builder-name-input" data-testid="jaldee-membership-template-builder-name-input" label="Template name" value={templateName} onChange={(event) => setTemplateName(event.target.value)} placeholder="Membership form" />
              <Input id="jaldee-membership-template-builder-description-input" data-testid="jaldee-membership-template-builder-description-input" label="Description" value={description} onChange={(event) => setDescription(event.target.value)} placeholder="Used for membership registration" />
            </div>
          </SectionCard>
          ...
          <SectionCard
            ...
            actions={
              <>
                <Button id="jaldee-membership-template-builder-add-field-button" data-testid="jaldee-membership-template-builder-add-field-button" size="sm" variant="outline" icon={<Plus size={14} />} onClick={() => openAddModal({ kind: "field" })}>
                  Input
                </Button>
                <Button id="jaldee-membership-template-builder-add-object-button" data-testid="jaldee-membership-template-builder-add-object-button" size="sm" variant="outline" icon={<Plus size={14} />} onClick={() => openAddModal({ kind: "object" })}>
                  Section
                </Button>
              </>
            }
          >
            <div className="flex flex-col gap-3">
              {items.map((item, index) => {
                const itemActions = (
                  <div className="flex shrink-0 items-center gap-1">
                    <Button id={`jaldee-membership-template-builder-item-${item.id}-move-up-button`} data-testid={`jaldee-membership-template-builder-item-${item.id}-move-up-button`} size="sm" variant="ghost" icon={<ArrowUp size={14} />} disabled={index === 0} onClick={() => setItems((current) => moveItem(current, index, -1))} aria-label="Move up" />
                    <Button id={`jaldee-membership-template-builder-item-${item.id}-move-down-button`} data-testid={`jaldee-membership-template-builder-item-${item.id}-move-down-button`} size="sm" variant="ghost" icon={<ArrowDown size={14} />} disabled={index === items.length - 1} onClick={() => setItems((current) => moveItem(current, index, 1))} aria-label="Move down" />
                    {item.kind === "field" ? (
                      <Button id={`jaldee-membership-template-builder-item-${item.id}-edit-button`} data-testid={`jaldee-membership-template-builder-item-${item.id}-edit-button`} size="sm" variant="ghost" icon={<Pencil size={14} />} onClick={() => openEditField(item.id, item.field)} aria-label="Edit input" />
                    ) : (
                      <Button id={`jaldee-membership-template-builder-item-${item.id}-edit-button`} data-testid={`jaldee-membership-template-builder-item-${item.id}-edit-button`} size="sm" variant="ghost" icon={<Pencil size={14} />} onClick={() => openEditObject(item.id, item.object)} aria-label="Edit section" />
                    )}
                    <Button id={`jaldee-membership-template-builder-item-${item.id}-delete-button`} data-testid={`jaldee-membership-template-builder-item-${item.id}-delete-button`} size="sm" variant="ghost" icon={<Trash2 size={14} />} onClick={() => setItems((current) => current.filter((currentItem) => currentItem.id !== item.id))} aria-label="Delete item" />
                  </div>
                );

                return (
                  <div key={item.id} data-testid={`jaldee-membership-template-builder-item-${item.id}-row`} className="rounded-lg border border-gray-200 bg-white p-3">
                    {item.kind === "field" ? (
                      <div className="flex items-start justify-between gap-3">
                        <FieldSummary field={item.field} testIdPrefix={`jaldee-membership-template-builder-item-${item.id}`} />
                        {itemActions}
                      </div>
                    ) : (
                      <div className="flex flex-col gap-3">
                        <div className="flex items-start justify-between gap-3">
                          <div data-testid={`jaldee-membership-template-builder-object-${item.object.id}-summary`} className="grid w-full min-w-0 gap-2 sm:grid-cols-[minmax(0,1fr),160px,120px] sm:items-center">
                            <div className="min-w-0">
                        ...
                        <div className="flex items-center justify-between border-t border-gray-100 pt-3">
                          ...
                          <Button id={`jaldee-membership-template-builder-object-${item.object.id}-add-field-button`} data-testid={`jaldee-membership-template-builder-object-${item.object.id}-add-field-button`} size="sm" variant="outline" icon={<Plus size={14} />} onClick={() => openAddModal({ kind: "objectField", objectId: item.object.id })}>
                            Input
                          </Button>
                        </div>

                        {item.object.fields.map((field) => (
                          <div key={field.id} data-testid={`jaldee-membership-template-builder-object-${item.object.id}-field-${field.id}-row`} className="rounded-md border border-gray-100 p-3">
                            <div className="flex items-center justify-between gap-3">
                              <FieldSummary field={field} testIdPrefix={`jaldee-membership-template-builder-object-${item.object.id}-field-${field.id}`} />
                              <div className="flex shrink-0 items-center gap-1">
                                <Button id={`jaldee-membership-template-builder-object-${item.object.id}-field-${field.id}-edit-button`} data-testid={`jaldee-membership-template-builder-object-${item.object.id}-field-${field.id}-edit-button`} size="sm" variant="ghost" icon={<Pencil size={14} />} onClick={() => openEditObjectField(item.object.id, field)} aria-label="Edit section input" />
                                <Button id={`jaldee-membership-template-builder-object-${item.object.id}-field-${field.id}-delete-button`} data-testid={`jaldee-membership-template-builder-object-${item.object.id}-field-${field.id}-delete-button`} size="sm" variant="ghost" icon={<Trash2 size={14} />} onClick={() => removeObjectField(item.object.id, field.id)} aria-label="Delete section input" />
                              </div>
                            </div>
                          </div>
                        ))}
              ...
              {items.length === 0 ? (
                <div data-testid="jaldee-membership-template-builder-items-empty-state" data-state="empty" className="rounded-lg border border-dashed border-gray-300 p-6 text-center text-sm text-gray-500">
        ...
        <SectionCard title="Generated JSON" actions={<Button id="jaldee-membership-template-builder-copy-json-button" data-testid="jaldee-membership-template-builder-copy-json-button" size="sm" variant="outline" icon={<Copy size={14} />} onClick={handleCopyJson}>Copy</Button>}>
          <pre data-testid="jaldee-membership-template-builder-json-preview" className="max-h-[720px] overflow-auto rounded-md bg-gray-950 p-4 text-xs leading-5 text-gray-50">{jsonPreview}</pre>
          <Textarea id="jaldee-membership-template-builder-create-payload-textarea" data-testid="jaldee-membership-template-builder-create-payload-textarea" className="mt-3 font-mono text-xs" label="Create payload" value={JSON.stringify(payload, null, 2)} readOnly rows={10} />
          {status ? <p data-testid="jaldee-membership-template-builder-status" data-state={isSaving ? "loading" : "status"} className="mt-3 text-sm text-gray-600">{status}</p> : null}
        </SectionCard>
      </div>
