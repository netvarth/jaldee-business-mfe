import { useMemo, useRef, useState } from "react";
import {
  Alert,
  Avatar,
  AvatarGroup,
  Badge,
  BarChart,
  BulkActionBar,
  Button,
  Checkbox,
  Combobox,
  ComponentErrorBoundary,
  ComparisonBarChart,
  ConfirmDialog,
  DataTable,
  DataTableToolbar,
  DatePicker,
  DatePickerPopover,
  MonthPicker,
  MonthPickerPopover,
  DateRangePicker,
  DescriptionList,
  Dialog,
  DialogFooter,
  Divider,
  Drawer,
  EmptyState,
  ErrorState,
  FileUpload,
  FormSection,
  Icon,
  Input,
  KanbanBoard,
  KpiStrip,
  LiveQueue,
  MultiCombobox,
  MultiLineChart,
  OtpInput,
  PageErrorBoundary,
  PageHeader,
  PhoneInput,
  Popover,
  PopoverSection,
  RadioGroup,
  RichTextEditor,
  SectionCard,
  Select,
  Skeleton,
  SkeletonCard,
  SkeletonTable,
  StatCard,
  Switch,
  Tabs,
  Textarea,
  TimePicker,
  Timeline,
  Tooltip,
  VitalsChart,
} from "../src";
import type {
  ColumnDef,
  DescriptionItem,
  KanbanColumn,
  KanbanItem,
  LiveQueueItem,
  PhoneInputValue,
  TimelineEvent,
  VitalPoint,
} from "../src";

type PatientRow = {
  id: string;
  name: string;
  doctor: string;
  status: string;
  due: string;
};

type BoardCard = KanbanItem & {
  title: string;
  owner: string;
  priority: string;
};

const tableRows: PatientRow[] = [
  { id: "P-1042", name: "Anita Rao", doctor: "Dr. Menon", status: "Active", due: "Today" },
  { id: "P-1047", name: "Ravi Das", doctor: "Dr. Joseph", status: "Queued", due: "11:30 AM" },
  { id: "P-1053", name: "Mina Paul", doctor: "Dr. Suma", status: "Review", due: "Tomorrow" },
  { id: "P-1061", name: "Niya Thomas", doctor: "Dr. Menon", status: "Active", due: "2:15 PM" },
  { id: "P-1068", name: "Akhil Nair", doctor: "Dr. Suma", status: "Queued", due: "4:00 PM" },
];

const timelineEvents: TimelineEvent[] = [
  { date: "09:15", title: "Patient registered", description: "Front desk completed intake.", variant: "info" },
  { date: "09:40", title: "Vitals captured", description: "BP and pulse logged into chart.", variant: "success" },
  { date: "10:05", title: "Lab pending", description: "CBC sample sent to lab queue.", variant: "warning" },
];

const detailItems: DescriptionItem[] = [
  { label: "Branch", value: "Thrissur" },
  { label: "Product Scope", value: "Health" },
  { label: "Owner", value: "Frontend Architecture Team" },
  { label: "Release Channel", value: "Preview" },
];

const kanbanColumns: KanbanColumn[] = [
  { id: "todo", label: "To do" },
  { id: "progress", label: "In progress" },
  { id: "done", label: "Done" },
];

const initialBoardItems: BoardCard[] = [
  { id: "task-1", columnId: "todo", title: "Patient list filters", owner: "Asha", priority: "High" },
  { id: "task-2", columnId: "progress", title: "Queue states", owner: "Nikhil", priority: "Medium" },
  { id: "task-3", columnId: "done", title: "Error boundaries", owner: "Niya", priority: "Done" },
];

const liveQueueItems: LiveQueueItem[] = [
  { id: "q-1", token: "A012", name: "Anita Rao", status: "in-progress", subtitle: "Dr. Menon", eta: "Now" },
  { id: "q-2", token: "A013", name: "Ravi Das", status: "critical", subtitle: "Triage override", eta: "2 min" },
  { id: "q-3", token: "A014", name: "Mina Paul", status: "waiting", subtitle: "General consult", eta: "9 min" },
];

const vitalsSeries: VitalPoint[] = [
  { label: "09:00", value: 96, timestamp: "09:00" },
  { label: "10:00", value: 98, timestamp: "10:00" },
  { label: "11:00", value: 101, timestamp: "11:00" },
  { label: "12:00", value: 99, timestamp: "12:00" },
  { label: "13:00", value: 103, timestamp: "13:00" },
];

const revenueTrend = [
  { label: "Mon", value: 42 },
  { label: "Tue", value: 58 },
  { label: "Wed", value: 37 },
  { label: "Thu", value: 66 },
  { label: "Fri", value: 74 },
];

const patientFlowTrend = [
  {
    key: "sat",
    fullLabel: "Saturday",
    values: { admitted: 2, reserved: 1, discharged: 1 },
  },
  {
    key: "sun",
    fullLabel: "Sunday",
    values: { admitted: 1, reserved: 2, discharged: 0 },
  },
  {
    key: "mon",
    fullLabel: "Monday",
    values: { admitted: 3, reserved: 2, discharged: 2 },
  },
  {
    key: "tue",
    fullLabel: "Tuesday",
    values: { admitted: 2, reserved: 3, discharged: 1 },
  },
];

const patientFlowSeries = [
  { key: "admitted", label: "Admitted", color: "#2563EB" },
  { key: "reserved", label: "Reserved", color: "#10B981" },
  { key: "discharged", label: "Discharged", color: "#F59E0B" },
];

const comparisonTrend = [
  { label: "Jan", value: 32 },
  { label: "Feb", value: 46 },
  { label: "Mar", value: 28 },
  { label: "Apr", value: 61 },
  { label: "May", value: 52 },
];

function PreviewGalleryCrash({ enabled }: { enabled: boolean }) {
  if (enabled) {
    throw new Error("Preview page crash test");
  }

  return (
    <div className="rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-800">
      Page content is healthy.
    </div>
  );
}

function PreviewWidgetCrash({ enabled }: { enabled: boolean }) {
  if (enabled) {
    throw new Error("Preview component crash test");
  }

  return (
    <div className="rounded-lg border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700">
      Widget content is healthy.
    </div>
  );
}

function PreviewFieldCard({
  title,
  type,
  description,
  snippet,
  children,
}: {
  title: string;
  type: string;
  description?: string;
  snippet: string;
  children: React.ReactNode;
}) {
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm md:p-6">
      <div className="mb-5 flex flex-wrap items-start justify-between gap-3">
        <div className="space-y-1">
          <p className="m-0 text-sm font-semibold text-slate-900">{title}</p>
          {description ? <p className="m-0 text-xs leading-5 text-slate-500">{description}</p> : null}
        </div>
        <Badge variant="neutral">{type}</Badge>
      </div>
      <div className="space-y-4">
        <div className="rounded-xl border border-slate-100 bg-slate-50/60 p-4 md:p-5">{children}</div>
        <div className="overflow-hidden rounded-xl border border-slate-200 bg-slate-950">
          <div className="border-b border-slate-800 px-4 py-2.5">
            <p className="m-0 text-[11px] font-bold uppercase tracking-[0.1em] text-slate-400">Usage Snippet</p>
          </div>
          <pre className="m-0 overflow-x-auto p-4 text-xs leading-6 text-slate-100">
            <code>{snippet}</code>
          </pre>
        </div>
      </div>
    </div>
  );
}

function PreviewFieldGroup({
  title,
  description,
  children,
}: {
  title: string;
  description: string;
  children: React.ReactNode;
}) {
  return (
    <div className="space-y-5 rounded-3xl border border-slate-200/80 bg-slate-50/80 p-5 md:p-6">
      <div className="space-y-1.5">
        <h3 className="m-0 text-base font-semibold text-slate-900">{title}</h3>
        <p className="m-0 text-sm leading-6 text-slate-500">{description}</p>
      </div>
      <div className="grid gap-5 xl:grid-cols-2">{children}</div>
    </div>
  );
}

export default function PreviewApp() {
  const [tab, setTab] = useState("overview");
  const [notify, setNotify] = useState(true);
  const [dialogOpen, setDialogOpen] = useState(false);
  const [confirmOpen, setConfirmOpen] = useState(false);
  const [drawerOpen, setDrawerOpen] = useState(false);
  const [datePopoverOpen, setDatePopoverOpen] = useState(false);
  const [selectedPopoverDate, setSelectedPopoverDate] = useState<Date | null>(new Date(2026, 2, 26));
  const [pageCrash, setPageCrash] = useState(false);
  const [componentCrash, setComponentCrash] = useState(false);
  const [boardItems, setBoardItems] = useState(initialBoardItems);
  const datePopoverAnchorRef = useRef<HTMLButtonElement | null>(null);

  const [selectedMonth, setSelectedMonth] = useState("2026-06");
  const [monthPopoverOpen, setMonthPopoverOpen] = useState(false);
  const [selectedPopoverMonth, setSelectedPopoverMonth] = useState<Date | null>(new Date(2026, 5, 1));
  const monthPopoverAnchorRef = useRef<HTMLButtonElement | null>(null);

  const [pageSize, setPageSize] = useState(10);
  const [currentPage, setCurrentPage] = useState(1);

  const [tableQuery, setTableQuery] = useState("");
  const [sortKey, setSortKey] = useState<string | null>(null);
  const [sortDir, setSortDir] = useState<"asc" | "desc">("asc");
  const [selectedRowKeys, setSelectedRowKeys] = useState<string[]>([]);
  const [otpValue, setOtpValue] = useState("123");
  const [phoneValue, setPhoneValue] = useState<PhoneInputValue>({
    countryCode: "+91",
    number: "9876543210",
    e164Number: "+919876543210",
  });
  const [multiComboboxValue, setMultiComboboxValue] = useState(["consult", "lab"]);
  const [richTextValue, setRichTextValue] = useState(
    '<h2 style="font-size:24px;">Service announcement</h2><p><strong>Initial assessment:</strong> patient is stable.</p><p style="line-height:1.8;">This block preserves spacing, line height, colors, tables, images, and inline styles in the saved HTML.</p><table style="width:100%;border-collapse:collapse;margin:12px 0;font-size:14px;"><tbody><tr><td style="border:1px solid #CBD5E1;padding:10px;min-width:96px;">Item</td><td style="border:1px solid #CBD5E1;padding:10px;min-width:96px;">Status</td></tr><tr><td style="border:1px solid #CBD5E1;padding:10px;min-width:96px;">Rich editor</td><td style="border:1px solid #CBD5E1;padding:10px;min-width:96px;">Preview ready</td></tr></tbody></table>'
  );

  const columns = useMemo<ColumnDef<PatientRow>[]>(
    () => [
      { key: "id", header: "ID", sticky: "left", width: 120, sortable: true },
      { key: "name", header: "Patient", sortable: true },
      { key: "doctor", header: "Doctor", sortable: true },
      {
        key: "status",
        header: "Status",
        sortable: true,
        render: (row) => (
          <Badge
            variant={
              row.status === "Active"
                ? "success"
                : row.status === "Queued"
                  ? "warning"
                  : "info"
            }
          >
            {row.status}
          </Badge>
        ),
      },
      { key: "due", header: "Next step", sortable: true, sticky: "right" },
    ],
    []
  );

  const filteredRows = useMemo(() => {
    const query = tableQuery.trim().toLowerCase();
    if (!query) return tableRows;

    return tableRows.filter((row) =>
      [row.id, row.name, row.doctor, row.status, row.due].some((value) =>
        value.toLowerCase().includes(query)
      )
    );
  }, [tableQuery]);

  const sortedRows = useMemo(() => {
    if (!sortKey) return filteredRows;

    const next = [...filteredRows].sort((a, b) => {
      const aValue = a[sortKey as keyof PatientRow];
      const bValue = b[sortKey as keyof PatientRow];

      return String(aValue ?? "").localeCompare(String(bValue ?? ""), undefined, {
        numeric: true,
        sensitivity: "base",
      });
    });

    return sortDir === "asc" ? next : next.reverse();
  }, [filteredRows, sortKey, sortDir]);
  const bulkCount = selectedRowKeys.length;

  return (
    <div className="min-h-screen px-6 py-8 lg:px-10">
      <div className="mx-auto max-w-7xl space-y-8">
        <PageHeader
          title="Design System Preview"
          breadcrumbs={[
            { label: "Packages", href: "#" },
            { label: "Design System" },
          ]}
          stepper={[
            { label: "Audit", description: "Compare doc and code", state: "complete" },
            { label: "Implement", description: "Fill component gaps", state: "current" },
            { label: "Document", description: "Update architecture doc", state: "upcoming" },
          ]}
          onNavigate={() => undefined}
          actions={
            <>
              <Button variant="outline" onClick={() => setDrawerOpen(true)}>
                Open Drawer
              </Button>
              <Button onClick={() => setDialogOpen(true)}>Open Dialog</Button>
            </>
          }
        />

        <PageHeader
          variant="navigation"
          title="Inventory Catalogs"
          back={{ href: "#", label: "Back to inventory" }}
          onNavigate={() => undefined}
        />

        <div className="grid gap-4 lg:grid-cols-[1.6fr,0.9fr]">
          <SectionCard
            title="Preview workspace"
            actions={<Badge variant="info">Live gallery</Badge>}
          >
            <div className="space-y-4">
              <p className="m-0 max-w-3xl text-sm leading-6 text-slate-600">
                This standalone page exercises the public components exported by
                <code className="mx-1 rounded bg-slate-100 px-1.5 py-0.5 text-xs">
                  @jaldee/design-system
                </code>
                so product teams can inspect layout, states, and interactions in one place.
              </p>
              <DescriptionList items={detailItems} columns={3} />
            </div>
          </SectionCard>

          <div className="grid gap-4 sm:grid-cols-2">
            <StatCard
              label="Components"
              value="40"
              trend={{ value: 12, direction: "up", label: "this sprint" }}
              icon="DS"
              accent="indigo"
            />
            <StatCard label="Status" value="Stable" icon="OK" accent="emerald" />
          </div>
        </div>

        <SectionCard title="Page Headers" description="Header variants used for primary pages and navigation contexts.">
          <div className="grid gap-5 xl:grid-cols-2">
            <PreviewFieldCard
              title="Primary Page Header"
              type="Page Header"
              description="Main page header with breadcrumbs, stepper, and actions."
              snippet={`<PageHeader
  title="Design System Preview"
  breadcrumbs={[
    { label: "Packages", href: "#" },
    { label: "Design System" },
  ]}
  stepper={[
    { label: "Audit", description: "Compare doc and code", state: "complete" },
    { label: "Implement", description: "Fill component gaps", state: "current" },
    { label: "Document", description: "Update architecture doc", state: "upcoming" },
  ]}
  actions={
    <>
      <Button variant="outline">Open Drawer</Button>
      <Button>Open Dialog</Button>
    </>
  }
/>`}
            >
              <PageHeader
                title="Design System Preview"
                breadcrumbs={[
                  { label: "Packages", href: "#" },
                  { label: "Design System" },
                ]}
                stepper={[
                  { label: "Audit", description: "Compare doc and code", state: "complete" },
                  { label: "Implement", description: "Fill component gaps", state: "current" },
                  { label: "Document", description: "Update architecture doc", state: "upcoming" },
                ]}
                onNavigate={() => undefined}
                actions={
                  <>
                    <Button variant="outline" onClick={() => setDrawerOpen(true)}>
                      Open Drawer
                    </Button>
                    <Button onClick={() => setDialogOpen(true)}>Open Dialog</Button>
                  </>
                }
              />
            </PreviewFieldCard>
            <PreviewFieldCard
              title="Navigation Header"
              type="Page Header"
              description="Compact navigation-oriented header with back affordance."
              snippet={`<PageHeader
  variant="navigation"
  title="Inventory Catalogs"
  back={{ href: "#", label: "Back to inventory" }}
/>`}
            >
              <PageHeader
                variant="navigation"
                title="Inventory Catalogs"
                back={{ href: "#", label: "Back to inventory" }}
                onNavigate={() => undefined}
              />
            </PreviewFieldCard>
          </div>
        </SectionCard>

        <SectionCard title="Primitives" description="Base building blocks with live output and insertion snippets.">
          <div className="grid gap-5 xl:grid-cols-2">
            <PreviewFieldCard
              title="Buttons"
              type="Action Control"
              description="Primary, secondary, outline, ghost, danger, and loading states."
              snippet={`<>
  <Button>Primary</Button>
  <Button variant="secondary">Secondary</Button>
  <Button variant="outline">Outline</Button>
  <Button variant="ghost">Ghost</Button>
  <Button variant="danger">Danger</Button>
  <Button loading>Loading</Button>
</>`}
            >
              <div className="flex flex-wrap items-center gap-3">
                <Button>Primary</Button>
                <Button variant="secondary">Secondary</Button>
                <Button variant="outline">Outline</Button>
                <Button variant="ghost">Ghost</Button>
                <Button variant="danger">Danger</Button>
                <Button loading>Loading</Button>
              </div>
            </PreviewFieldCard>
            <PreviewFieldCard
              title="Badges"
              type="Status Indicator"
              description="Semantic variants, dots, and count badges."
              snippet={`<>
  <Badge variant="success" dot>Ready</Badge>
  <Badge variant="warning" dot>Queued</Badge>
  <Badge variant="danger" dot>Blocked</Badge>
  <Badge variant="info">Info</Badge>
  <Badge variant="neutral">Neutral</Badge>
  <Badge variant="info" count={12}>Unread</Badge>
</>`}
            >
              <div className="flex flex-wrap items-center gap-3">
                <Badge variant="success" dot>Ready</Badge>
                <Badge variant="warning" dot>Queued</Badge>
                <Badge variant="danger" dot>Blocked</Badge>
                <Badge variant="info">Info</Badge>
                <Badge variant="neutral">Neutral</Badge>
                <Badge variant="info" count={12}>Unread</Badge>
              </div>
            </PreviewFieldCard>
            <PreviewFieldCard
              title="Avatars"
              type="Identity Display"
              description="Single avatars at multiple sizes and grouped participants."
              snippet={`<>
  <Avatar name="Asha Menon" size="sm" />
  <Avatar name="Ravi Das" size="md" />
  <Avatar name="Mina Joseph" size="lg" />
  <AvatarGroup users={[{ name: "Asha Menon" }, { name: "Ravi Das" }]} />
</>`}
            >
              <div className="flex items-center gap-4">
                <Avatar name="Asha Menon" size="sm" />
                <Avatar name="Ravi Das" size="md" />
                <Avatar name="Mina Joseph" size="lg" />
                <AvatarGroup
                  users={[
                    { name: "Asha Menon" },
                    { name: "Ravi Das" },
                    { name: "Mina Joseph" },
                    { name: "Nikhil Varma" },
                  ]}
                />
              </div>
            </PreviewFieldCard>
            <PreviewFieldCard
              title="Icons"
              type="Icon Set"
              description="Named glyphs from the shared icon registry."
              snippet={`<>
  <Icon name="search" />
  <Icon name="calendar" />
  <Icon name="filter" />
  <Icon name="chart" />
</>`}
            >
              <div className="flex flex-wrap items-center gap-3">
                {(["search", "calendar", "filter", "chart", "warehouse", "alert"] as const).map((name) => (
                  <span
                    key={name}
                    className="inline-flex h-9 w-9 items-center justify-center rounded-md border border-slate-200 bg-white text-slate-600"
                    title={name}
                  >
                    <Icon name={name} />
                  </span>
                ))}
              </div>
            </PreviewFieldCard>
            <PreviewFieldCard
              title="Alerts"
              type="Feedback Banner"
              description="Inline success and warning notifications."
              snippet={`<>
  <Alert variant="success" title="Release healthy">
    Preview build is rendering with Tailwind and design tokens.
  </Alert>
  <Alert variant="warning" title="Check contrast">
    Review neutral text and disabled states before promoting new tokens.
  </Alert>
</>`}
            >
              <div className="space-y-4">
                <Alert variant="success" title="Release healthy">
                  Preview build is rendering with Tailwind and design tokens.
                </Alert>
                <Alert variant="warning" title="Check contrast">
                  Review neutral text and disabled states before promoting new tokens.
                </Alert>
              </div>
            </PreviewFieldCard>
            <PreviewFieldCard
              title="Tooltip and Popover"
              type="Contextual Overlay"
              description="Hover help and compact contextual content."
              snippet={`<>
  <Tooltip content="Tooltip preview">
    <Button variant="secondary">Hover for tooltip</Button>
  </Tooltip>

  <Popover trigger={<Button variant="outline">Open Popover</Button>}>
    <PopoverSection>
      <p>Queue context</p>
      <p>Use popovers for compact contextual actions and metadata.</p>
    </PopoverSection>
  </Popover>
</>`}
            >
              <div className="space-y-4">
                <Divider />
                <div className="flex flex-wrap items-center gap-3">
                  <Tooltip content="Tooltip preview">
                    <Button variant="secondary">Hover for tooltip</Button>
                  </Tooltip>
                  <Popover trigger={<Button variant="outline">Open Popover</Button>}>
                    <PopoverSection>
                      <p className="m-0 text-sm font-semibold text-gray-900">Queue context</p>
                      <p className="m-0 text-sm text-gray-500">
                        Use popovers for compact contextual actions and metadata.
                      </p>
                    </PopoverSection>
                  </Popover>
                </div>
              </div>
            </PreviewFieldCard>
          </div>
        </SectionCard>

        <SectionCard
          title="Form Controls"
          description="Examples are grouped by control family and each card shows the field type for faster scanning."
        >
          <div className="space-y-6">
            <PreviewFieldGroup
              title="Text Entry"
              description="Single-line and multi-line text inputs, including prefixed and read-only variants."
            >
              <PreviewFieldCard
                title="Patient Name"
                type="Input Text"
                description="Basic single-line text field."
                snippet={`<Input
  label="Patient name"
  placeholder="Enter full name"
  hint="Shown on the patient chart"
/>`}
              >
                <Input label="Patient name" placeholder="Enter full name" hint="Shown on the patient chart" />
              </PreviewFieldCard>
              <PreviewFieldCard
                title="Fee"
                type="Input With Prefix"
                description="Currency prefix with numeric-style entry."
                snippet={`<Input
  label="Fee"
  prefix="Rs."
  placeholder="750"
/>`}
              >
                <Input label="Fee" prefix="Rs." placeholder="750" />
              </PreviewFieldCard>
              <PreviewFieldCard
                title="Doctor"
                type="Read Only Input"
                description="Read-only text field for locked values."
                snippet={`<Input
  label="Doctor"
  value="Dr. Menon"
  readOnly
/>`}
              >
                <Input label="Doctor" value="Dr. Menon" readOnly />
              </PreviewFieldCard>
              <PreviewFieldCard
                title="Clinical Note"
                type="Textarea"
                description="Multi-line text entry for notes and descriptions."
                snippet={`<Textarea
  label="Clinical note"
  hint="Use this to preview multi-line input styling."
  defaultValue="Follow-up visit scheduled for review after lab results."
/>`}
              >
                <Textarea
                  label="Clinical note"
                  hint="Use this to preview multi-line input styling."
                  defaultValue="Follow-up visit scheduled for review after lab results."
                />
              </PreviewFieldCard>
            </PreviewFieldGroup>

            <PreviewFieldGroup
              title="Selection Controls"
              description="Dropdowns, searchable selects, multi-select controls, and choice-based inputs."
            >
              <PreviewFieldCard
                title="Department"
                type="Dropdown"
                description="Standard single-select dropdown."
                snippet={`<Select
  label="Department"
  placeholder="Select department"
  defaultValue=""
  options={[
    { value: "op", label: "Outpatient" },
    { value: "ip", label: "Inpatient" },
    { value: "lab", label: "Laboratory" },
  ]}
/>`}
              >
                <Select
                  label="Department"
                  placeholder="Select department"
                  hint="Used for routing and queue grouping"
                  defaultValue=""
                  options={[
                    { value: "op", label: "Outpatient" },
                    { value: "ip", label: "Inpatient" },
                    { value: "lab", label: "Laboratory" },
                  ]}
                  data-testid="preview-department-select"
                />
              </PreviewFieldCard>
              <PreviewFieldCard
                title="Assigned Services"
                type="Multi Select Dropdown"
                description="Native multi-select dropdown pattern."
                snippet={`<Select
  label="Assigned services"
  multiple
  defaultValue={["consult", "lab"]}
  options={[
    { value: "consult", label: "Consultation" },
    { value: "lab", label: "Lab work" },
    { value: "pharmacy", label: "Pharmacy" },
  ]}
/>`}
              >
                <Select
                  label="Assigned services"
                  hint="Native multi-select preview"
                  multiple
                  defaultValue={["consult", "lab"]}
                  options={[
                    { value: "consult", label: "Consultation" },
                    { value: "lab", label: "Lab work" },
                    { value: "pharmacy", label: "Pharmacy" },
                    { value: "billing", label: "Billing" },
                  ]}
                  data-testid="preview-services-select"
                />
              </PreviewFieldCard>
              <PreviewFieldCard
                title="Escalation Path"
                type="Dropdown With Error"
                description="Validation-state example for select controls."
                snippet={`<Select
  label="Escalation path"
  error="Please choose an escalation path"
  defaultValue=""
  options={[
    { value: "doctor", label: "Doctor" },
    { value: "nurse", label: "Nursing desk" },
    { value: "billing", label: "Billing" },
  ]}
/>`}
              >
                <Select
                  label="Escalation path"
                  error="Please choose an escalation path"
                  defaultValue=""
                  options={[
                    { value: "doctor", label: "Doctor" },
                    { value: "nurse", label: "Nursing desk" },
                    { value: "billing", label: "Billing" },
                  ]}
                  data-testid="preview-escalation-select"
                />
              </PreviewFieldCard>
              <PreviewFieldCard
                title="Consulting Doctor"
                type="Searchable Dropdown"
                description="Combobox for search-first single selection."
                snippet={`<Combobox
  label="Consulting doctor"
  placeholder="Choose doctor"
  options={[
    { value: "menon", label: "Dr. Menon", description: "General medicine" },
    { value: "joseph", label: "Dr. Joseph", description: "Cardiology" },
  ]}
/>`}
              >
                <Combobox
                  label="Consulting doctor"
                  placeholder="Choose doctor"
                  options={[
                    { value: "menon", label: "Dr. Menon", description: "General medicine" },
                    { value: "joseph", label: "Dr. Joseph", description: "Cardiology" },
                    { value: "suma", label: "Dr. Suma", description: "Paediatrics" },
                  ]}
                />
              </PreviewFieldCard>
              <PreviewFieldCard
                title="Care Services"
                type="Searchable Multi Select"
                description="Search-driven multi-select combobox."
                snippet={`<MultiCombobox
  label="Care services"
  value={multiComboboxValue}
  onValueChange={setMultiComboboxValue}
  options={[
    { value: "consult", label: "Consultation" },
    { value: "lab", label: "Lab work" },
    { value: "pharmacy", label: "Pharmacy" },
  ]}
/>`}
              >
                <MultiCombobox
                  label="Care services"
                  hint="Searchable multi-select preview"
                  value={multiComboboxValue}
                  onValueChange={setMultiComboboxValue}
                  options={[
                    { value: "consult", label: "Consultation", description: "Doctor visit" },
                    { value: "lab", label: "Lab work", description: "Diagnostics" },
                    { value: "pharmacy", label: "Pharmacy", description: "Medicines" },
                    { value: "billing", label: "Billing", description: "Payment desk" },
                  ]}
                />
              </PreviewFieldCard>
              <PreviewFieldCard
                title="Priority"
                type="Radio Group"
                description="Mutually exclusive radio selection."
                snippet={`<RadioGroup
  label="Priority"
  name="priority"
  value="urgent"
  onChange={() => undefined}
  options={[
    { value: "routine", label: "Routine" },
    { value: "urgent", label: "Urgent" },
    { value: "critical", label: "Critical" },
  ]}
/>`}
              >
                <RadioGroup
                  label="Priority"
                  name="priority"
                  value="urgent"
                  onChange={() => undefined}
                  options={[
                    { value: "routine", label: "Routine" },
                    { value: "urgent", label: "Urgent" },
                    { value: "critical", label: "Critical" },
                  ]}
                />
              </PreviewFieldCard>
              <PreviewFieldCard
                title="Lead Priority"
                type="Segmented Radio Group"
                description="Segmented option set for compact state switching."
                snippet={`<RadioGroup
  label="Lead priority"
  name="lead-priority"
  variant="segmented"
  value="normal"
  onChange={() => undefined}
  options={[
    { value: "low", label: "LOW" },
    { value: "normal", label: "NORMAL" },
    { value: "high", label: "HIGH" },
  ]}
/>`}
              >
                <RadioGroup
                  label="Lead priority"
                  name="lead-priority"
                  variant="segmented"
                  value="normal"
                  onChange={() => undefined}
                  options={[
                    { value: "low", label: "LOW" },
                    { value: "normal", label: "NORMAL" },
                    { value: "high", label: "HIGH" },
                    { value: "urgent", label: "URGENT" },
                  ]}
                />
              </PreviewFieldCard>
            </PreviewFieldGroup>

            <PreviewFieldGroup
              title="Date, Time, and Verification"
              description="Scheduling, date-range, secure verification, and phone-specific entry patterns."
            >
              <PreviewFieldCard
                title="Phone Number"
                type="Phone Input"
                description="Country-aware phone number field."
                snippet={`<PhoneInput
  label="Phone number"
  value={phoneValue}
  onChange={setPhoneValue}
  hint="intl-tel-input backed phone field"
/>`}
              >
                <PhoneInput
                  label="Phone number"
                  value={phoneValue}
                  onChange={setPhoneValue}
                  hint="intl-tel-input backed phone field"
                />
              </PreviewFieldCard>
              <PreviewFieldCard
                title="Verification Code"
                type="OTP Input"
                description="Multi-slot one-time-password field."
                snippet={`<OtpInput
  label="Verification code"
  value={otpValue}
  onChange={setOtpValue}
  hint="Paste or type one-time codes"
/>`}
              >
                <OtpInput
                  label="Verification code"
                  value={otpValue}
                  onChange={setOtpValue}
                  hint="Paste or type one-time codes"
                />
              </PreviewFieldCard>
              <PreviewFieldCard
                title="Visit Date"
                type="Date Picker"
                description="Calendar date selection inside a field."
                snippet={`<DatePicker
  label="Visit date"
  defaultValue="2026-03-26"
/>`}
              >
                <DatePicker label="Visit date" defaultValue="2026-03-26" />
              </PreviewFieldCard>
              <PreviewFieldCard
                title="Date Picker Popover"
                type="Popover Date Picker"
                description="Date picker triggered from a standalone button."
                snippet={`const anchorRef = useRef<HTMLButtonElement | null>(null);
const [open, setOpen] = useState(false);
const [selectedDate, setSelectedDate] = useState<Date | null>(new Date());

<Button ref={anchorRef} onClick={() => setOpen(true)}>
  {selectedDate?.toLocaleDateString() ?? "Choose date"}
</Button>

{open ? (
  <DatePickerPopover
    selectedDate={selectedDate}
    anchorRect={anchorRef.current?.getBoundingClientRect() ?? null}
    anchorRef={anchorRef}
    onSelectDate={(date) => {
      setSelectedDate(date);
      setOpen(false);
    }}
    onClose={() => setOpen(false)}
  />
) : null}`}
              >
                <div className="flex flex-col gap-1.5">
                  <span className="ds-form-label">Date picker popover</span>
                  <Button
                    ref={datePopoverAnchorRef}
                    type="button"
                    variant="outline"
                    onClick={() => setDatePopoverOpen(true)}
                  >
                    {selectedPopoverDate?.toLocaleDateString() ?? "Choose date"}
                  </Button>
                </div>
              </PreviewFieldCard>
              <PreviewFieldCard
                title="Scheduled Month"
                type="Month Picker"
                description="Month-level selection in an input field."
                snippet={`<MonthPicker
  label="Scheduled month"
  value={selectedMonth}
  onChange={(e) => setSelectedMonth(e.target.value)}
  hint={\`Selected value: \${selectedMonth}\`}
/>`}
              >
                <MonthPicker
                  label="Scheduled month"
                  value={selectedMonth}
                  onChange={(e) => setSelectedMonth(e.target.value)}
                  hint={`Selected value: ${selectedMonth}`}
                />
              </PreviewFieldCard>
              <PreviewFieldCard
                title="Month Picker Popover"
                type="Popover Month Picker"
                description="Month selection from a popover button trigger."
                snippet={`const monthAnchorRef = useRef<HTMLButtonElement | null>(null);
const [monthOpen, setMonthOpen] = useState(false);
const [selectedMonthDate, setSelectedMonthDate] = useState<Date | null>(new Date());

<Button ref={monthAnchorRef} onClick={() => setMonthOpen(true)}>
  Choose month
</Button>

{monthOpen ? (
  <MonthPickerPopover
    selectedDate={selectedMonthDate}
    anchorRect={monthAnchorRef.current?.getBoundingClientRect() ?? null}
    anchorRef={monthAnchorRef}
    onSelectMonth={(date) => {
      setSelectedMonthDate(date);
      setMonthOpen(false);
    }}
    onClose={() => setMonthOpen(false)}
    onClear={() => setSelectedMonthDate(null)}
  />
) : null}`}
              >
                <div className="flex flex-col gap-1.5">
                  <span className="ds-form-label">Month picker popover</span>
                  <Button
                    ref={monthPopoverAnchorRef}
                    type="button"
                    variant="outline"
                    onClick={() => setMonthPopoverOpen(true)}
                  >
                    {selectedPopoverMonth
                      ? `${selectedPopoverMonth.toLocaleString("default", { month: "long" })}, ${selectedPopoverMonth.getFullYear()}`
                      : "Choose month"}
                  </Button>
                </div>
              </PreviewFieldCard>
              <PreviewFieldCard
                title="Admission Window"
                type="Date Range Picker"
                description="Start and end date selection in one control."
                snippet={`<DateRangePicker
  label="Admission window"
  defaultValue={{ start: "2026-03-26", end: "2026-03-29" }}
/>`}
              >
                <DateRangePicker
                  label="Admission window"
                  defaultValue={{ start: "2026-03-26", end: "2026-03-29" }}
                />
              </PreviewFieldCard>
              <PreviewFieldCard
                title="Consultation Slot"
                type="Time Picker"
                description="Time-specific scheduling input."
                snippet={`<TimePicker
  label="Consultation slot"
  defaultValue="10:30"
/>`}
              >
                <TimePicker label="Consultation slot" defaultValue="10:30" />
              </PreviewFieldCard>
            </PreviewFieldGroup>

            <PreviewFieldGroup
              title="Binary and Upload Controls"
              description="On-off toggles, boolean checks, and file selection patterns."
            >
              <PreviewFieldCard
                title="Attachments"
                type="File Upload"
                description="Attachment input with file constraints."
                snippet={`<FileUpload
  label="Attachments"
  accept=".pdf,.png"
  maxSize={2 * 1024 * 1024}
  onUpload={() => undefined}
/>`}
              >
                <FileUpload
                  label="Attachments"
                  accept=".pdf,.png"
                  maxSize={2 * 1024 * 1024}
                  onUpload={() => undefined}
                />
              </PreviewFieldCard>
              <PreviewFieldCard
                title="Email Summary"
                type="Checkbox"
                description="Single boolean checkbox selection."
                snippet={`<Checkbox
  label="Send discharge summary by email"
  defaultChecked
/>`}
              >
                <Checkbox label="Send discharge summary by email" defaultChecked />
              </PreviewFieldCard>
              <PreviewFieldCard
                title="Enable Reminders"
                type="Switch"
                description="Immediate on-off toggle control."
                snippet={`<Switch
  label="Enable reminders"
  checked={notify}
  onChange={setNotify}
/>`}
              >
                <Switch label="Enable reminders" checked={notify} onChange={setNotify} />
              </PreviewFieldCard>
              <PreviewFieldCard
                title="Patient Intake Layout"
                type="Form Section"
                description="Grouped section wrapper showing how multiple inputs compose in a structured form."
                snippet={`<FormSection
  title="Patient intake"
  description="Representative validation and helper states."
>
  <Input label="Patient name" placeholder="Enter full name" />
  <Select label="Department" options={departmentOptions} />
  <Textarea label="Notes" defaultValue="Grouped fields inside FormSection." />
</FormSection>`}
              >
                <FormSection
                  title="Patient intake"
                  description="Representative validation and helper states."
                >
                  <Input label="Patient name" placeholder="Enter full name" hint="Shown on the patient chart" />
                  <Select
                    label="Department"
                    placeholder="Select department"
                    defaultValue=""
                    options={[
                      { value: "op", label: "Outpatient" },
                      { value: "ip", label: "Inpatient" },
                      { value: "lab", label: "Laboratory" },
                    ]}
                  />
                  <Textarea label="Notes" defaultValue="Grouped fields inside FormSection." />
                </FormSection>
              </PreviewFieldCard>
            </PreviewFieldGroup>
          </div>
        </SectionCard>

        <SectionCard
          title="Rich Text Editor"
          description="Wide authoring surface with grouped controls, usage code, and rendered HTML output."
        >
          <div className="grid gap-5">
            <PreviewFieldCard
              title="Authoring Canvas"
              type="Rich Text Editor"
              description="Full-width editor with grouped formatting controls and persistent HTML output."
              snippet={`<RichTextEditor
  label="Authoring canvas"
  value={richTextValue}
  onChange={setRichTextValue}
  hint="Use the grouped toolbar to format content, insert images, tables, horizontal/vertical lines, and preserve spacing."
  minHeightClassName="min-h-[460px]"
  fullWidth
  testId="preview-rich-text-editor"
/>`}
            >
              <RichTextEditor
                label="Authoring canvas"
                value={richTextValue}
                onChange={setRichTextValue}
                hint="Use the grouped toolbar to format content, insert images, tables, horizontal/vertical lines, and preserve spacing."
                minHeightClassName="min-h-[460px]"
                fullWidth
                testId="preview-rich-text-editor"
              />
            </PreviewFieldCard>
            <PreviewFieldCard
              title="Rendered Output"
              type="HTML Preview"
              description="Saved HTML rendered as product output with raw markup shown below."
              snippet={`<div
  dangerouslySetInnerHTML={{
    __html: richTextValue || "<p>Preview appears here...</p>",
  }}
/>`}
            >
              <div className="flex min-h-[720px] w-full flex-col overflow-hidden rounded-[var(--radius-control)] border border-slate-200 bg-white shadow-sm">
                <div className="flex items-center justify-between border-b border-slate-200 bg-slate-50 px-4 py-3">
                  <div>
                    <p className="m-0 text-sm font-semibold text-slate-900">Rendered Preview</p>
                    <p className="m-0 text-xs text-slate-500">This is how the saved HTML displays.</p>
                  </div>
                  <Badge variant="info">HTML</Badge>
                </div>
                <div
                  className="prose max-w-none flex-1 overflow-auto p-6 text-sm leading-6 text-slate-800"
                  style={{ whiteSpace: "pre-wrap", wordBreak: "break-word" }}
                  dangerouslySetInnerHTML={{ __html: richTextValue || "<p style='color:#94A3B8;'>Preview appears here...</p>" }}
                />
                <div className="border-t border-slate-200 bg-slate-950 p-4">
                  <p className="m-0 mb-2 text-xs font-bold uppercase tracking-[0.1em] text-slate-400">Saved HTML</p>
                  <pre className="m-0 max-h-44 overflow-auto whitespace-pre-wrap break-words text-xs leading-5 text-slate-100">
                    {richTextValue || "<empty>"}
                  </pre>
                </div>
              </div>
            </PreviewFieldCard>
          </div>
        </SectionCard>

        <SectionCard title="Feedback and Overlay" description="States, loading patterns, and overlay triggers with usage guidance.">
          <div className="grid gap-5 xl:grid-cols-2">
            <PreviewFieldCard
              title="Empty State"
              type="State Feedback"
              description="Empty-result treatment with optional call to action."
              snippet={`<EmptyState
  title="No appointments scheduled"
  description="Create a booking to populate this state."
  action={<Button size="sm">New appointment</Button>}
/>`}
            >
              <EmptyState
                title="No appointments scheduled"
                description="Create a booking to populate this state."
                action={<Button size="sm">New appointment</Button>}
              />
            </PreviewFieldCard>
            <PreviewFieldCard
              title="Error State"
              type="State Feedback"
              description="Error treatment with recovery action."
              snippet={`<ErrorState
  title="Queue service unavailable"
  description="Realtime updates could not be loaded."
  action={<Button variant="outline" size="sm">Retry</Button>}
/>`}
            >
              <ErrorState
                title="Queue service unavailable"
                description="Realtime updates could not be loaded."
                action={<Button variant="outline" size="sm">Retry</Button>}
              />
            </PreviewFieldCard>
            <PreviewFieldCard
              title="Dialog, Confirm, and Drawer Triggers"
              type="Overlay Trigger"
              description="Buttons that open shared overlay components."
              snippet={`<>
  <Button onClick={() => setDialogOpen(true)}>Dialog</Button>
  <Button variant="secondary" onClick={() => setConfirmOpen(true)}>
    Confirm dialog
  </Button>
  <Button variant="outline" onClick={() => setDrawerOpen(true)}>
    Drawer
  </Button>
</>`}
            >
              <div className="flex flex-wrap gap-3">
                <Button onClick={() => setDialogOpen(true)}>Dialog</Button>
                <Button variant="secondary" onClick={() => setConfirmOpen(true)}>
                  Confirm dialog
                </Button>
                <Button variant="outline" onClick={() => setDrawerOpen(true)}>
                  Drawer
                </Button>
              </div>
            </PreviewFieldCard>
            <PreviewFieldCard
              title="Skeletons"
              type="Loading Placeholder"
              description="Card and table-like loading placeholders."
              snippet={`<>
  <SkeletonCard />
  <div>
    <Skeleton height={16} width="45%" />
    <SkeletonTable rows={3} columns={3} />
  </div>
</>`}
            >
              <div className="grid gap-4 md:grid-cols-2">
                <SkeletonCard />
                <div className="rounded-lg border border-slate-200 bg-white p-4">
                  <Skeleton height={16} width="45%" />
                  <div className="mt-3">
                    <SkeletonTable rows={3} columns={3} />
                  </div>
                </div>
              </div>
            </PreviewFieldCard>
          </div>
        </SectionCard>

        <SectionCard title="Navigation and Layout" description="Tabbed navigation and layout containers with direct insertion references.">
          <div className="grid gap-5 xl:grid-cols-2">
            <PreviewFieldCard
              title="Tabs"
              type="Navigation"
              description="Top-level tab navigation with optional counts."
              snippet={`<Tabs
  value={tab}
  onValueChange={setTab}
  items={[
    { value: "overview", label: "Overview", count: 12 },
    { value: "activity", label: "Activity", count: 4 },
    { value: "settings", label: "Settings" },
  ]}
/>`}
            >
              <Tabs
                value={tab}
                onValueChange={setTab}
                items={[
                  { value: "overview", label: "Overview", count: 12 },
                  { value: "activity", label: "Activity", count: 4 },
                  { value: "settings", label: "Settings" },
                ]}
              />
            </PreviewFieldCard>
            <PreviewFieldCard
              title="Stat Cards"
              type="Summary Layout"
              description="Compact KPI blocks for dashboards and overview pages."
              snippet={`<StatCard
  label="Patients today"
  value="128"
  trend={{ value: 8, direction: "up", label: "vs yesterday" }}
  icon="PT"
  accent="indigo"
/>`}
            >
              <div className="grid gap-4 md:grid-cols-2">
                <StatCard
                  label="Patients today"
                  value="128"
                  trend={{ value: 8, direction: "up", label: "vs yesterday" }}
                  icon="PT"
                  accent="indigo"
                />
                <StatCard
                  label="Avg wait"
                  value="14 min"
                  trend={{ value: 6, direction: "down", label: "queue improved" }}
                  icon="WT"
                  accent="amber"
                />
                <StatCard label="Alerts" value="3" icon="AL" accent="rose" />
                <StatCard label="Rooms free" value="11" icon="RM" accent="emerald" />
              </div>
            </PreviewFieldCard>
            <PreviewFieldCard
              title="Section Card"
              type="Layout Container"
              description="Contained layout block with title and optional action."
              snippet={`<SectionCard
  title="Contained card"
  actions={<Button size="sm" variant="ghost">Action</Button>}
>
  <p>Section cards are useful for dense enterprise screens.</p>
</SectionCard>`}
            >
              <SectionCard
                title="Contained card"
                actions={<Button size="sm" variant="ghost">Action</Button>}
              >
                <p className="m-0 text-sm leading-6 text-slate-600">
                  Section cards are useful for dense enterprise screens with multiple independently
                  refreshable blocks.
                </p>
              </SectionCard>
            </PreviewFieldCard>
          </div>
        </SectionCard>

        <SectionCard title="Data Display" description="Data surfaces, charts, and boards with example usage snippets.">
          <div className="grid gap-5 xl:grid-cols-2">
            <PreviewFieldCard
              title="KPI Strip"
              type="Data Summary"
              description="Inline KPI collection with icon accents."
              snippet={`<KpiStrip
  items={[
    { label: "Visits", value: 128, icon: <Icon name="chart" />, accent: "indigo" },
    { label: "Revenue", value: "Rs. 42k", icon: <Icon name="trend" />, accent: "emerald" },
  ]}
/>`}
            >
              <KpiStrip
                items={[
                  { label: "Visits", value: 128, icon: <Icon name="chart" />, accent: "indigo" },
                  { label: "Revenue", value: "Rs. 42k", icon: <Icon name="trend" />, accent: "emerald" },
                  { label: "Alerts", value: 3, icon: <Icon name="alert" />, accent: "rose" },
                ]}
              />
            </PreviewFieldCard>
            <PreviewFieldCard
              title="Bulk Action Bar"
              type="Table Utility"
              description="Selection-aware actions for batch operations."
              snippet={`<BulkActionBar
  count={selectedRowKeys.length}
  onClear={() => setSelectedRowKeys([])}
  actions={[
    { label: "Assign", onClick: () => undefined, variant: "secondary" },
    { label: "Archive", onClick: () => undefined, variant: "outline" },
    { label: "Delete", onClick: () => undefined, variant: "danger" },
  ]}
/>`}
            >
              <BulkActionBar
                count={bulkCount || 3}
                onClear={() => setSelectedRowKeys([])}
                actions={[
                  { label: "Assign", onClick: () => undefined, variant: "secondary" },
                  { label: "Archive", onClick: () => undefined, variant: "outline" },
                  { label: "Delete", onClick: () => undefined, variant: "danger" },
                ]}
              />
            </PreviewFieldCard>
            <PreviewFieldCard
              title="Data Table Toolbar"
              type="Table Utility"
              description="Search and count toolbar paired with data grids."
              snippet={`<DataTableToolbar
  query={tableQuery}
  onQueryChange={setTableQuery}
  searchPlaceholder="Search patients, doctor, status..."
  recordCount={filteredRows.length}
/>`}
            >
              <DataTableToolbar
                query={tableQuery}
                onQueryChange={(value) => {
                  setTableQuery(value);
                  setCurrentPage(1);
                }}
                searchPlaceholder="Search patients, doctor, status..."
                recordCount={filteredRows.length}
              />
            </PreviewFieldCard>
            <PreviewFieldCard
              title="Data Table"
              type="Data Grid"
              description="Sortable, selectable, paginated data table."
              snippet={`<DataTable
  data={sortedRows}
  columns={columns}
  getRowId={(row) => row.id}
  sorting={{ sortKey, sortDir, onChange: handleSort }}
  selection={{ selectedRowKeys, onChange: setSelectedRowKeys }}
  pagination={{ pageSize, total, page, onChange: setCurrentPage }}
/>`}
            >
              <DataTable
                data={sortedRows}
                columns={columns}
                getRowId={(row) => row.id}
                sorting={{
                  sortKey,
                  sortDir,
                  onChange: (key, dir) => {
                    setSortKey(key);
                    setSortDir(dir);
                    setCurrentPage(1);
                  },
                }}
                selection={{
                  selectedRowKeys,
                  onChange: setSelectedRowKeys,
                }}
                pagination={{
                  pageSize,
                  total: filteredRows.length,
                  page: currentPage,
                  onChange: setCurrentPage,
                  onPageSizeChange: (size) => {
                    setPageSize(size);
                    setCurrentPage(1);
                  },
                }}
              />
            </PreviewFieldCard>
            <PreviewFieldCard
              title="Description List"
              type="Metadata Display"
              description="Key-value metadata layout for read-only detail views."
              snippet={`<DescriptionList items={detailItems} columns={2} />`}
            >
              <DescriptionList items={detailItems} columns={2} />
            </PreviewFieldCard>
            <PreviewFieldCard
              title="Timeline"
              type="Event Sequence"
              description="Chronological event feed with statuses."
              snippet={`<Timeline events={timelineEvents} />`}
            >
              <Timeline events={timelineEvents} />
            </PreviewFieldCard>
            <PreviewFieldCard
              title="Bar Chart"
              type="Chart"
              description="Single-series bar chart with menu affordance."
              snippet={`<BarChart
  data={revenueTrend}
  showMenuIcon
  yAxisLabel="Rs."
/>`}
            >
              <BarChart data={revenueTrend} showMenuIcon yAxisLabel="Rs." />
            </PreviewFieldCard>
            <PreviewFieldCard
              title="Comparison Chart"
              type="Chart"
              description="Single-value comparison bars across periods."
              snippet={`<ComparisonBarChart data={comparisonTrend} />`}
            >
              <ComparisonBarChart data={comparisonTrend} />
            </PreviewFieldCard>
            <PreviewFieldCard
              title="Multi Line Chart"
              type="Chart"
              description="Multi-series trend visualization."
              snippet={`<MultiLineChart
  data={patientFlowTrend}
  series={patientFlowSeries}
  chartHeight={260}
/>`}
            >
              <MultiLineChart
                data={patientFlowTrend}
                series={patientFlowSeries}
                chartHeight={260}
              />
            </PreviewFieldCard>
            <PreviewFieldCard
              title="Live Queue"
              type="Operational Widget"
              description="Realtime queue state with next/recall/hold actions."
              snippet={`<LiveQueue
  currentToken="A012"
  currentLabel="Anita Rao"
  items={liveQueueItems}
  onCallNext={() => undefined}
  onRecall={() => undefined}
  onHold={() => undefined}
/>`}
            >
              <LiveQueue
                currentToken="A012"
                currentLabel="Anita Rao"
                items={liveQueueItems}
                onCallNext={() => undefined}
                onRecall={() => undefined}
                onHold={() => undefined}
              />
            </PreviewFieldCard>
            <PreviewFieldCard
              title="Vitals Chart"
              type="Operational Widget"
              description="Threshold-aware line chart for monitoring readings."
              snippet={`<VitalsChart
  title="Temperature"
  unit="F"
  series={vitalsSeries}
  minThreshold={97}
  maxThreshold={102}
/>`}
            >
              <VitalsChart
                title="Temperature"
                unit="F"
                series={vitalsSeries}
                minThreshold={97}
                maxThreshold={102}
              />
            </PreviewFieldCard>
            <PreviewFieldCard
              title="Kanban Board"
              type="Board Layout"
              description="Draggable cards across workflow columns."
              snippet={`<KanbanBoard
  columns={kanbanColumns}
  items={boardItems}
  onDragEnd={(itemId, newColumnId) => updateBoard(itemId, newColumnId)}
  renderCard={(item) => <BoardCard item={item} />}
/>`}
            >
              <KanbanBoard<BoardCard>
                columns={kanbanColumns}
                items={boardItems}
                onDragEnd={(itemId, newColumnId) => {
                  setBoardItems((current) =>
                    current.map((item) =>
                      item.id === itemId ? { ...item, columnId: newColumnId } : item
                    )
                  );
                }}
                renderCard={(item) => (
                  <div className="rounded-lg border border-slate-200 bg-white p-3 shadow-sm">
                    <p className="m-0 text-sm font-semibold text-slate-900">{item.title}</p>
                    <p className="mt-1 text-xs text-slate-500">{item.owner}</p>
                    <div className="mt-3">
                      <Badge
                        variant={
                          item.priority === "High"
                            ? "danger"
                            : item.priority === "Done"
                              ? "success"
                              : "warning"
                        }
                      >
                        {item.priority}
                      </Badge>
                    </div>
                  </div>
                )}
              />
            </PreviewFieldCard>
          </div>
        </SectionCard>

        <SectionCard title="Error Boundaries" description="Page and widget-level crash handling patterns with snippets.">
          <div className="grid gap-5 xl:grid-cols-2">
            <PreviewFieldCard
              title="Page Error Boundary"
              type="Error Recovery"
              description="Wraps an entire page region and catches runtime failures."
              snippet={`<PageErrorBoundary>
  <PreviewGalleryCrash enabled={pageCrash} />
</PageErrorBoundary>`}
            >
              <div className="space-y-4">
                <div className="flex items-center gap-3">
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => setPageCrash((value) => !value)}
                  >
                    {pageCrash ? "Reset page demo" : "Trigger page error"}
                  </Button>
                </div>
                <PageErrorBoundary>
                  <PreviewGalleryCrash enabled={pageCrash} />
                </PageErrorBoundary>
              </div>
            </PreviewFieldCard>
            <PreviewFieldCard
              title="Component Error Boundary"
              type="Error Recovery"
              description="Isolates failures to a single widget without breaking the full page."
              snippet={`<ComponentErrorBoundary label="Preview widget">
  <PreviewWidgetCrash enabled={componentCrash} />
</ComponentErrorBoundary>`}
            >
              <div className="space-y-4">
                <div className="flex items-center gap-3">
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => setComponentCrash((value) => !value)}
                  >
                    {componentCrash ? "Reset widget demo" : "Trigger component error"}
                  </Button>
                </div>
                <ComponentErrorBoundary label="Preview widget">
                  <PreviewWidgetCrash enabled={componentCrash} />
                </ComponentErrorBoundary>
              </div>
            </PreviewFieldCard>
          </div>
        </SectionCard>
      </div>

      <Dialog
        open={dialogOpen}
        onClose={() => setDialogOpen(false)}
        title="Dialog Preview"
        description="Use this to validate modal layout and footer actions."
      >
        <p className="m-0 text-sm leading-6 text-slate-600">
          This sample modal uses the public dialog primitives exported by the design system package.
        </p>
        <DialogFooter>
          <Button variant="secondary" onClick={() => setDialogOpen(false)}>
            Close
          </Button>
          <Button onClick={() => setConfirmOpen(true)}>Continue</Button>
        </DialogFooter>
      </Dialog>

      <ConfirmDialog
        open={confirmOpen}
        onClose={() => setConfirmOpen(false)}
        onConfirm={() => setConfirmOpen(false)}
        title="Confirm preview action"
        description="This is a representative destructive confirmation flow."
        confirmVariant="danger"
        confirmLabel="Delete record"
      />

      <Drawer open={drawerOpen} onClose={() => setDrawerOpen(false)} title="Preview Drawer">
        <div className="space-y-4">
          <Alert variant="info" title="Drawer content">
            Use this panel to inspect overlay, spacing, and stacked actions.
          </Alert>
          <Input label="Assignee" defaultValue="Asha Menon" />
          <Textarea
            label="Notes"
            defaultValue="Prepare a tighter empty-state variant for mobile layouts."
          />
          <div className="flex justify-end gap-3">
            <Button variant="secondary" onClick={() => setDrawerOpen(false)}>
              Cancel
            </Button>
            <Button onClick={() => setDrawerOpen(false)}>Save</Button>
          </div>
        </div>
      </Drawer>

      {datePopoverOpen ? (
        <DatePickerPopover
          selectedDate={selectedPopoverDate}
          anchorRect={datePopoverAnchorRef.current?.getBoundingClientRect() ?? null}
          anchorRef={datePopoverAnchorRef}
          onSelectDate={(date) => {
            setSelectedPopoverDate(date);
            setDatePopoverOpen(false);
          }}
          onClose={() => setDatePopoverOpen(false)}
        />
      ) : null}

      {monthPopoverOpen ? (
        <MonthPickerPopover
          selectedDate={selectedPopoverMonth}
          anchorRect={monthPopoverAnchorRef.current?.getBoundingClientRect() ?? null}
          anchorRef={monthPopoverAnchorRef}
          onSelectMonth={(date) => {
            setSelectedPopoverMonth(date);
            setMonthPopoverOpen(false);
          }}
          onClose={() => setMonthPopoverOpen(false)}
          onClear={() => {
            setSelectedPopoverMonth(null);
          }}
        />
      ) : null}
    </div>
  );
}
